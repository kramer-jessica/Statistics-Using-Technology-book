...
Is there a relationship between the alcohol content and the number of calories in 12-ounce beer? To determine if there is one, we explore a sample of 227 beers' alcohol content and their calories (Find Out How Many Calories in Beer?, 2019). @tbl-Beer shows the first five rows of the dataset. 
...
Click to expand the box below to see instructions to import and view the [beer_data]{.data} dataset in your Rguroo account.
...
1. Open your Rguroo account.  
2. Go to the **Data** toolbox.  
3. From the [Data Import]{.dpd} dropdown, select [Dataset Repository]{.fun}.  
4. In the top search box, type [kozak]{.typein}, then select the [Statistics Using Technology – Kozak]{.repo} repository.  
5. In the middle search box, type [beer]{.typein}, and select the [beer_data]{.data} dataset that appears in the lower panel.  
6. Click the [Import]{.button} button. The dataset will be imported into your Rguroo account.  
7. Click the [Close]{.button} button to close the Rguroo dialog.  
8. To view the dataset, double-click [beer_data]{.data} under the **Data** toolbox list.
...
Click to expand the box below see how to create the scatterplot of calories vs. alcohol content in Rguroo.

::: {.callout-note appearance="simple" collapse="true" icon="none" title="![](Rguroo_Icons/R_circle_logo.svg){width=22px style='vertical-align:middle;'} Creating Scatterplot of Calories vs. Alcohol Content in Rguroo"}

**Before you begin:** Make sure you have already imported the [beer_data]{.data} dataset into your Rguroo account, as was shown [here](#Importing-Beer_data-to-Rguroo).

1. Go to your Rguroo account.  
2. Open the **Plots** toolbox.  
3. Click on the [Create Plot]{.dpd} dropdown, and select [Scatterplot]{.fun}. This opens the Scatterplot dialog.  
4. From the [Dataset]{.dpd} dropdown, select the [beer_data]{.data} dataset.  
5. From the [Predictor (x)]{.dpd} dropdown, select the [alcohol]{.var} variable.  
6. From the [Response (y)]{.dpd} dropdown, select the [calories]{.var} variable.  
7. **(Optional)** In the **Label** section of the dialog:  
   - In the [Title]{.des} textbox, enter [Calories versus Alcohol Content in Beer]{.typein}.  
   - In the [X-Axis]{.des} textbox, enter [Alcohol Content]{.typein}.  
   - In the [Y-Axis]{.des} textbox, enter [Number of Calories]{.typein}.  
8. Click the preview icon ![preview icon](Rguroo_Icons/preview_inline.png) to see a preview of the scatterplot.

::: {.callout-note appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}

![Scatterplot dialog in Rguroo](Rguroo_images/Regression/Scatterplot_dialog.png){width=500px}
...
Click to expand the box below to see how to add a regression line to the scatterplot in Rguroo.


::: {.callout-note appearance="simple" collapse="true" icon="none" title="![](Rguroo_Icons/R_circle_logo.svg){width=22px style='vertical-align:middle;'} Adding Regression Line to Scatterplot in Rguroo"}

**Before you begin:** Ensure you’ve created the scatterplot of calories vs. alcohol content in your Rguroo account, as shown [here](#Creating-Scatterplot-of-Calories-vs-Alcohol-Content-in-Rguroo).

1. Go to the Rguroo tab with your scatterplot of calories vs. alcohol content.  
2. Open the [Basics]{.diag} dialog by clicking the ![](Rguroo_Icons/basics.png) button.  
3. Select the checkbox [LS Line]{.des} in the **Superimpose** section of the dialog.  
...
![Scatteplot Dialog: Adding a least squares line](Rguroo_outputs/Regression/Scatterplot_dialog_LS.png){width=500px}

:::

:::

```{r beer10-point-lm, echo=FALSE, evaluate=FALSE}
...
4. _Normality_: For inference on parameters to be valid, the $y$-values for each fixed value of $x$ should follow a normal distribution. 


Since it can be difficult to verify these conditions directly, we will show how you can use the following to assess these conditions graphically:

   i. _Linearity:_ Check whether the scatterplot has a roughly linear pattern. 

   ii. _Constant Variance_: Examine the plot of the residuals for randomness. If you observe a pattern, this assumption may not hold.  

   iii. _Normality_: Inspect the normal probability plot of the residuals to see if it is approximately linear.
...
Click to expand the box below to see how to obtain the regression equation in Rguroo.

::: {.callout-note appearance="simple" collapse="true" icon="none" title="![](Rguroo_Icons/R_circle_logo.svg){width=22px style='vertical-align:middle;'} Finding the Regression Equation in Rguroo"}
**Before you begin:** Make sure you have already imported the [beer_data]{.data} dataset into your Rguroo account, as was shown [here](#Importing-Beer_data-to-Rguroo).

1. Go to your Rguroo account.
2. Open the **Analytics** toolbox.
3. From the [Analysis]{.dpd} dropdown, select [Linear Regression]{.fun} --> [Simple Regression]{.fun}. This opens the Simple Linear Regression dialog.
4. From the [Dataset]{.dpd} dropdown, select the [beer_data]{.data} dataset.
5. From the [Predictor (x)]{.dpd} dropdown, select the [alcohol]{.var} variable.
6. From the [Response (y)]{.dpd} dropdown, select the [calories]{.var} variable.
7. Click the preview icon ![preview icon](Rguroo_Icons/preview_inline.png) to see a preview of the regression results.

::: {.callout-note appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
![Simple Linear Regression dialog in Rguroo](Rguroo_outputs/Regression/AlcoholCalorieRegress_dialog.png){width=500px}
:::
...
i.  From @fig-Beer-point, the scatter plot looks fairly linear. Note that this graph is also shown in the output of the [Linear Regression]{.fun} function in Rguroo.

```{r AlcoholCalorieResvsFit}
...
ii. @fig-Beer-resvsfit shows a plot of residuals versus fitted values. To graph the residuals, the residuals must first be calculated. For each $x$ value, compute the corresponding $y$ value predicted by the regression equation; this predicted value is called the *fitted value* and is denoted by $\hat{y}$. Then subtract the observed $y$ value from this fitted value to obtain the residual. Fortunately, Rguroo calculates these automatically for you.

Click to expand the box below to see how to obtain the residuals in Rguroo.

::: {.callout-note appearance="simple" collapse="true" icon="none" title="![](Rguroo_Icons/R_circle_logo.svg){width=22px style='vertical-align:middle;'} Obtaining Residuals in Rguroo"}

**Before you begin:** Continue with the steps shown [here](#Finding-the-Regression-Equation-in-Rguroo) to obtain the regression equation in Rguroo.

1. Go to the tab in Rguroo with your regression results.
...
::: {.callout-note appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
![Simple Linear Regression dialog: Obtaining Residuals](Rguroo_outputs/Regression/AlcoholCalorieResidualsDialog1.png){width=500px}
...
```{r chapter-10-beer-residuals, echo=FALSE, evaluate=FALSE}
