...
a. Using the data in [beer_data]{.data}, construct a scatterplot with alcohol content on the horizontal axis and number of calories on the vertical axis.
...
(a) To aid in figuring out if there is a relationship between the alcohol content and the number of calories, we draw a scatterplot of the data. First, it is helpful to clearly state the variables. Following the convention of algebra, we represent the explanatory variable as $x$ and the response variable as $y$. In this example, $x$ = percent alcohol content in the beer; [alcohol]{.var}, and $y$ = number of calories in 12 ounce beer; [calories]{.var}
...
Again, to predict the number of calories for a beer with alcohol conten of 14%, we substitute 14 in for $x$ in the regression equation.
...
1.  When an anthropologist finds skeletal remains, one task is to estimate the person’s height. Data on height (in cm) and the length of metacarpal bone 1 (in mm) were collected and are shown in @tbl-Metacarpal (“Prediction of Height,” 2013).
    ...
    a. Create a scatterplot with metacarpal length as the predictor and and height on as the response. Then draw the least squares regression line on the same graph so that you can see how well the line fits the data. Does the pattern look approximately linear.
...
2. @tbl-House contains a portion of the dataset [house]{.data} that contains data on house value and the annual rental income each house generates (“Capital and Rental,” 2013).
    ...
    a. Create a plot of rental income versus house value, using house value as the predictor variable and annual rental income as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.
    ...
    g. Interpret the intercept in context. Is the $y$-intercept meaningful in this situation? Explain.
...
6. Data on body weights and brain weights are given in the dataset [body]{.data}, part of which is shown in @tbl-Body ("Brain2bodyweight," 2013).
    ...
    a. Create a plot of brain weight versus body weight, using body weight (in kg) as the predictor variable and brain weight (in g) as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.
...
8. Per capita income in 1960 dollars for European countries and the percent of the labor force that works in agriculture in 1960 are given in the dataset [agriculture]{.data} ("OECD economic development," 2013). The first six rows of this dataset are shown in @tbl-Agriculture.
    ...
    a. Create a plot of per capita income versus percent of labor force in agriculture, using percent of labor force in agriculture as the predictor variable and per capita income (in 1960 dollars) as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.
...
9. Cigarette smoking and cancer have been linked. The number of deaths per one hundred thousand from bladder cancer and the number of cigarettes sold per capita in 1960 are given in the dataset [cancer_1]{.data} ("Smoking and cancer," 2013). The first six rows of the data are shown in @tbl-Cancerbladder.
    ...
    a. Create a plot of deaths from bladder cancer versus number of cigarettes sold per capita, using number of cigarettes sold per capita as the predictor variable and number of deaths per one hundred thousand from bladder cancer as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.
...
10. The weight of a car can influence the mileage that the car can obtain. A random sample of cars' weights and mileage was collected and reported in the dataset [cars]{.data}, with the first six rows shown in @tbl-Cars ("us auto mileage," 2019).
    ...
    a. Create a plot of mileage versus car weight, using car weight (in pounds) as the predictor variable and mileage (in miles per gallon) as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.
...
12. In a laboratory experiment, participants are randomly assigned to consume either a caffeinated beverage or a caffeine-free beverage that looks and tastes the same. After 30 minutes, reaction times are measured. Participants who consumed caffeine have faster reaction times on average. Does this study provide evidence that caffeine causes faster reaction times? Explain your answer.
