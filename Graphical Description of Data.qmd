...
4.  Qualitative data: analysis of the data **MJ: What do you mean?**
...
Remember, qualitative data are usually words describing a characteristic of the individual. Once you’ve gathered raw data, the real work begins. A long list of observations doesn’t tell you much on its own. Raw data is usually messy, unstructured, and difficult to understand or use. The next step is to organize the data in a way that reveals patterns and meaning. A simple way to organize qualitative data is to create a frequency distribution or relative frequency distribution. Both of these are tables which organize non‑numerical categories into groups and shows how often each category occurs.

A **frequency distribution for qualitative data** is a table that lists each category with its corresponding frequency. The **frequency** of a category is the **number of observations** that fall into that category.

A **relative frequency distribution for qualitative data** is a table that lists each category with its corresponding relative frequency. Relative frequency is the **proportion of observations** that fall into a category. Relative frequencies can be in the form of a proportion or a percentage.
...
Data was collected for two semesters in a statistics class. @tbl-Class shows the variables and the first 6 rows of the data set. The data sets are usually larger than what is shown. Create a frequency distribution and relative frequency distribution for vehicle type. **MJ I think we should bring the codebook right after the dataset upload or even here that we are showing a portion of the data**
:::
...
::: {#Importing-Class_Survey1-to-Rguroo .callout-note appearance="simple" collapse="true" icon="none" title="![](Rguroo_Icons/R_circle_logo.svg){width=22px style='vertical-align:middle;'}  Importing class_survey1 to Rguroo"}
...
5.  Click the preview icon ![preview icon](Rguroo_Icons/preview_inline.png) to see a preview of the frequecy distribution.

::: {.callout-tip .inner-callout appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
![Data Tabulation dialog showing settings for creating frequency distribtion of vehicle type.](Rguroo_dialogs/Graphical_Description_of_Data/dialog_frequency_distribution_vehicle.png){width="500px"}
:::
::::

![Frequency Distribution for Vehicle Type](Rguroo_outputs/Graphical_Description_of_Data/frequency_distribution_vehicle.png){width="80%" fig-alt="a list of vehicles with the corresponding counts"}

![Relative Frequency Distribution for Vehicle Type](Rguroo_outputs/Graphical_Description_of_Data/rel_frequency_distribution_vehicle.png){width="80%" fig-alt="a list of vehicles with the corresponding proportions"}
...
**Bar graphs or Bar charts** consist of the frequencies or relative frequencies on one axis and the categories on the other axis. Drawing the bar graph using Rguroo is performed by importing the dataset (or creating a dataset) and using the Plots toolbox in Rguroo.

::: {#exm-drawing-a-bar-chart}
## Drawing a Bar Chart

Use the [class_survey1]{.data} to create a bar chart of vehicle type.
:::
...
**Optional** You can change the title and names for the x and y axes in the Barplot dialog box. To do this, simply type in the titles for each in the **Label** boxes. **MJ I think while it is optional, students should be encouraged to put meaningful labeles and title. It would be a good idea that in this plot we also add appropriate labels**
...
@fig-class_data is a Bar graph with bars for Audi, Buick, Honda, Hyundai, Mercury, Nissan with height of 1, Dodge and None with height of 2, Jeep, Subaru, Toyota with heights of 3, and Chevrolet and Ford at height of 4.
...
Use the [class_survey1]{.data} to create a bar chart of vehicle type by gender.
:::
...
:::: {#Creating-Bar-Graph-of-Vehicle-Gender-in-Rguroo .callout-note appearance="simple" collapse="true" icon="none" title="![](Rguroo_Icons/R_circle_logo.svg){width=22px style='vertical-align:middle;'} Creating Bar Graph of Vehicle Type and Gender in Rguroo"}
**Before you begin:** Make sure you have already imported the [class_survey1]{.data} dataset into your Rguroo account, as was shown [here](#Importing-Class_Survey1-to-Rguroo).
...
6.  Click the preview icon ![preview icon](Rguroo_Icons/preview_inline.png) to see a preview of the bar graph.

::: {.callout-tip .inner-callout appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
![Bar Graph dialog showing settings for creating bar graph of vehicle type by gender.](Rguroo_dialogs/Graphical_Description_of_Data/dialog_vehicle_gender_2.png){width="500px"}
:::
::::
...
Use the [class_survey1]{.data} to create a relative frequency bar chart of vehicle type.
:::
...
:::: {#Creating-Relative-Frequency-Bar-Graph-of-Vehicle-in-Rguroo .callout-note appearance="simple" collapse="true" icon="none" title="![](Rguroo_Icons/R_circle_logo.svg){width=22px style='vertical-align:middle;'} Creating Relative Frequency Bar Graph of Vehicle Type in Rguroo"}
**Before you begin:** Make sure you have already imported the [class_survey1]{.data} dataset into your Rguroo account, as was shown [here](#Importing-Class_Survey1-to-Rguroo).
...
::: {.callout-tip .inner-callout appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
![Bar Graph dialog showing settings for creating relative frequency bar graph of vehicle type.](Rguroo_dialogs/Graphical_Description_of_Data/dialog_rel_freq_vehicle_2.png){width="500px"}
:::
::::
...
@fig-class_data_dot-car is a realtive frequency bar graph of vehicles showing the vehicle types on the horizontal axis and proportions (relative frequency) on the vertical axis.

Notice that a frequency bar graph and relative frequency bar graph give you similar information.

**MJ I think it is also instructive to give the relative frequency bargraph when comparing male and females, since if the number of males and females are not equal, this graph would make more sense and the difference should be pointed out between the relative frequency abd frequency bar graph for this case. A stacked barplot would vbe good to have.**
...
Use the [class_survey1]{.data} to create a pie chart of vehicle type.
:::
...
@fig-class_data_point is a pie chart of type of vehicles owned by students (Toyota, Subaru, none, Nissan, Mercury, Jeep, Honda, Ford, Dodge, Chevrolet, and Buick, Hyundai, and Audi). Each vehicle percentage is displayed in the legend on the right.
...
There are many other types of graphs that can be used on qualitative data. There are spreadsheet software packages that will create most of them, and it is better to look at them to see how to create them. It depends on your data as to which may be useful, but the frequecy bar graph, relative frequency bar graph, and pie chart are the most useful.
...
![Rguroo dataset logo](Rguroo_Icons/Rguroo_repo_logo.png) The dataset for this exercise is available in the Rguroo dataset repository [Kozak]{.repo}, with the dataset name [Eyeglasses]{.data}.
:::
...
As with qualitative data, it is also advantageous to organize quantitative data since the long list of raw data usually does not provide insight into the sample or population.

Frequency and relative frequecy distributions can also be used on quantitative data, where the categories are numerical values that occur in the data with their corresponding frequency counts or proportions. However, most dataset values are varied where there are too many different data values and a regular frequency or relative frequeny distribution would be meaningless.

For quantitative data that is largely spread, we can group the data values into classes, also known as binning. When you group data into classes/bins, you create a **grouped frequency distribution**.

Lower and upper class limits define the boundaries of each class in a grouped frequency distribution. The **lower class limit** is the smallest value that can belong to a class, while the **upper class limit** is the largest value that can belong to that same class. Together, these limits specify exactly which data values fall into each interval. Most of the time, classes are displayed using interval notation. For example, suppose we wanted to group the ages of students into a group from 20-25. This could be displayed using interval notation, \[20,25). This interval would capture every student whos age is 20 to 25 years, including 20 years old, but not 25 years old. These limits keep the classes organized, prevent overlap, and ensure every data point has a clear place within the distribution.

Each class has a **class width**, and an associated size of the class (or the number of values within the class). If you have a grouped frequency distribution, the class width can be determined by subtracting the lower-class limit from the upper-class limit.
...
Click to expand the box below see how to create a grouped frequency distribution for distance to campus.
...
![Grouped Frequency Distribution for Distance to Campus](Rguroo_outputs/Graphical_Description_of_Data/gfd_distance.png){width="80%" fig-alt="a list of vehicles with the corresponding proportions"}

The grouped frequency distribution above used the default method, Freedman-Diaconis, creating 11 classes. Each class has a class width of 5. Now that the data is organized, it is easily seen that most students live within 5 miles of campus. There are also two students who live farther from campus than the rest of the class: one lives between 25 and 30 miles from campus, and another lives between 50 and 55 miles from campus.
...
Data was collected for two semesters in a statistics class drive. The data frame is in @tbl-Class and the code book is below the data frame. **MJ Need to link specifically to the codebook when referencing**
...
**MJ We should add title and label for axes**
...
:::: {#Creating-Density-Plot-in-Rguroo .callout-note appearance="simple" collapse="true" icon="none" title="![](Rguroo_Icons/R_circle_logo.svg){width=22px style='vertical-align:middle;'} Creating Density Plot in Rguroo"}
**Before you begin:** Make sure you have already imported the [class_survey1]{.data} dataset into your Rguroo account, as was shown [here](#Importing-Class_Survey1-to-Rguroo).
...
::: {.callout-tip .inner-callout appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
![Density dialog showing settings for creating density plot for distance to campus.](Rguroo_dialogs/Graphical_Description_of_Data/dialog_distance_density.png){width="500px"}
:::
::::
...
When you look at a distribution, look at the basic shape. There are some basic shapes that are seen in histograms. Realize though that some distributions have no shape. The common shapes are bell-shape, skewed, and uniform. Another interest is how many peaks a graph may have. This is known as modal.

A symmetric shape means that you can fold the graph in half down the middle and the two sides will line up. You can think of the two sides as being mirror images of each other.A special case of a symmetric shape is a bell shape, where the graph has a single peak in the center and the two sides decrease smoothly and evenly as you move away from that center. Skewed means one "tail" of the graph is longer than the other. The graph is skewed in the direction of the longer tail (backwards from what you would expect). A uniform graph has all the bars the same height. **MJ The description of skewed needs more clarification. Also, it would be good to have graphs to show examples that clarifies the ideas heer. I see that this happens later in Section 2.2.2**
...
<!--- MJ Thisis incorrect. I replaced it: Usually if a graph has more than two peaks, the modal information is not longer of interest. -->
...
![Bell Shape Graph](Rguroo_outputs/Graphical_Description_of_Data/symmetric.png){width="60%" fig-alt="one side looks like other"}

A bell-shape histogram has a balanced, mirror‑like appearance, with the bars rising to a peak near the center and then decreasing at roughly the same rate on both sides. The left and right halves of the distribution look similar in both height and spread, so if you folded the graph down the middle, the two sides would line up closely. There are no long tails pulling the shape in one direction, and the overall pattern often forms a smooth, mound‑shaped curve. Note that bell-shape doesn’t require perfect symmetry.

**Bimodal**

This graph is bimodal:

![Bimodal Graph](Rguroo_outputs/Graphical_Description_of_Data/bimodal_symmetric.png){width="60%" fig-alt="two bars high, and one side looks like other"}

A bimodal histogram has two distinct peaks, meaning the bars rise to form one cluster of high values, dip down in the middle, and then rise again to form a second cluster. Visually, the histogram looks like a pair of hills or humps separated by a noticeable valley. In this histogram, the overall shape is symmetric; however, this is not a requirement for a bimodal graph. In addition, the separation between the peaks can be sharp or gradual.

**Skewed Right**

This graph is skewed to the right:

![Skewed Right Graph](Rguroo_outputs/Graphical_Description_of_Data/skewed_right.png){width="60%" fig-alt="small bars on right"}

A skewed right histogram has most of its highest bars on the left side, with the heights tapering off gradually as you move to the right. The bulk of the data forms a tall block on the left, while a long, stretched‑out tail extends as you move right.The most frequently occuring data sits closer to the left, and the right tail is noticeably longer and thinner than the left.

**Graph: Skewed Left Distribution**

This graph is skewed to the left:

![Skewed Left Graph](Rguroo_outputs/Graphical_Description_of_Data/skewed_left2.png){width="60%" fig-alt="small bars on left"}

A skewed left histogram has most of its highest bars on the right side, with the heights tapering off gradually as you move to the left. The bulk of the data forms a tall block on the right, while a long, stretched‑out tail extends as you move left.The most frequently occuring data sits closer to the right, and the left tail is noticeably longer and thinner than the left.

**Uniform Distribution**

This graph is uniform:

![Uniform Graph](Rguroo_outputs/Graphical_Description_of_Data/uniform.png){width="60%" fig-alt="All bars same height"}

A uniform histogram has bars that are all roughly the same height, giving the graph a flat, even appearance across the entire range of values. No single interval stands out as especially common or rare; instead, the data are spread evenly, with each bar occurring at about the same frequency. Visually, it looks like a row of nearly level rectangles, without peaks, clusters, or noticeable patterns. Note that the graph above is perfectly uniform (all heights of the bars are equal); however, this is not a requirement to be uniform.
...
::: {#Importing-Tuition-4-Year-to-Rguroo .callout-note appearance="simple" collapse="true" icon="none" title="![](Rguroo_Icons/R_circle_logo.svg){width=22px style='vertical-align:middle;'}  Importing tuition_4_year to Rguroo"}
1.  Open the **Data** toolbox in Rguroo.\
2.  From the [Data Import]{.dpd} dropdown, select [Dataset Repository]{.fun}.\
3.  In the top search box, type [kozak]{.typein}, then select the [Statistics Using Technology – Kozak]{.repo} repository.\
...
::: {.callout-tip .inner-callout appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
![Density dialog showing settings for creating density plot of instate tuition.](Rguroo_dialogs/Graphical_Description_of_Data/dialog_density_tuition_instate_2.png){width="500px"}
:::
::::
...
This denisty plot of instate tuition shows the highest peak on left, then a dip and up to a smaller peak in the middle that is lower than the left peak. Then the bar heights gradually decrease as you move to the right. Due to the characteristics of the graph, the distribution is skewed right.
...
There are many other types of graphs. Some of the more common ones are the scatter plot (point plot), and a time-series plot. There are also many different graphs that have emerged lately for qualitative data. Many are found in publications and websites. The following is a description of a scatterplot, and the time-series plot.

### Scatter Plot or Point Plot

Sometimes you have two different variables and you want to see if they are related in any way. A scatter plot helps you to see what the relationship would look like. A scatter plot is plotting the ordered pairs on the cartesian coordinate system (x-y axis).

::: {#exm-scatter-plot}
## Creating a Scatter Plot

Data was collected from a sleep study conducted on college students. Is there a relationship between average amount of sleep and GPA? To answer this question, we can look at data where these two variables were collected and examine the data on a scatterplot.
:::

**Solution**

Click to expand the box below see how to import and view the [SleepStudy]{.data}. dataset.

::: {#Importing-Sleep-Study-Data .callout-note appearance="simple" collapse="true" icon="none" title="![](Rguroo_Icons/R_circle_logo.svg){width=22px style='vertical-align:middle;'}  Importing SleepStudy data to Rguroo"}
1.  Open the **Data** toolbox in Rguroo.\
2.  From the [Data Import]{.dpd} dropdown, select [Dataset Repository]{.fun}.\
3.  In the top search box, type [kozak]{.typein}, then select the [Statistics Using Technology – Kozak]{.repo} repository.\
4.  In the middle search box, type [sleep]{.typein}, and select the [SleepStudy]{.data} dataset that appears in the lower panel.\
5.  Click the [Import]{.button}. The dataset will be imported into your Rguroo account.\
6.  Click the [Close]{.button} to close the dialog.\
7.  To view the dataset, double-click [SleepStudy]{.data} under the **Data** toolbox list.
:::

**MJ Is there a codebook for this datatse?**
...
```{r, echo=FALSE}
#| tbl-alt: "Table showing first 6 rows of sample from Pulse_Rate dataset"
#| label: tbl-sleepstudy
#| tbl-cap: "Head of SleepStudy Data frame"
...
Let x=explanatory variable = Average sleep per night (in hours)

y=response variable = GPA

Click to expand the box below see how to create a scatterplot of average sleep per night versus GPA.
...
**Before you begin:** Make sure you have already imported the [SleepStudy]{.data} dataset into your Rguroo account, as was shown [here](#Importing-Sleep-Study).

1.  Open the **Plots** toolbox in Rguroo.\
2.  Open the [Create Plot]{.dpd} dropdown, and select [Scatter Plot]{.fun}. This opens the Scatter Plot dialog.\
3.  In the Scatter Plot dialog, choose the [SleepStudy]{.data} dataset from the [Dataset]{.dpd} dropdown.\
4.  Choose the [AverageSleep]{.var} variable from the [Predictor (x)]{.dpd} dropdown.
5.  Choose the [GPA]{.var} variable from the [Response (y)]{.dpd} dropdown.
6.  Click the preview icon ![preview icon](Rguroo_Icons/preview_inline.png) to see a preview of the scatter plot.

::: {.callout-tip .inner-callout appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
![Scatter plot dialog showing settings for creating scatter plot of average sleep versus GPA.](Rguroo_dialogs/Graphical_Description_of_Data/dialog_sleepstudy.png){width="500px"}
:::
::::
...
#| fig-alt: "See the description of the graph below the graph"
#| warning: FALSE
#| label: fig-sleep-study
#| fig-cap: "GPA versus Average Sleep"
knitr::include_graphics("Rguroo_outputs/Graphical_Description_of_Data/scatterplot_sleep_study.svg")
```

Description of @fig-sleep-study is a scatter plot with dots all over the plot.

Looking at the graph @fig-sleep-study there does not appear to be a linear relationship between average sleep per night and GPA.
...
::: {#Importing-Austrailian-Financial-to-Rguroo .callout-note appearance="simple" collapse="true" icon="none" title="![](Rguroo_Icons/R_circle_logo.svg){width=22px style='vertical-align:middle;'}  Importing Australian_financial to Rguroo"}
1.  Open the **Data** toolbox in Rguroo.\
2.  From the [Data Import]{.dpd} dropdown, select [Dataset Repository]{.fun}.\
3.  In the top search box, type [kozak]{.typein}, then select the [Statistics Using Technology – Kozak]{.repo} repository.\
...
**Before you begin:** Make sure you have already imported the [Australian_financial]{.data} dataset into your Rguroo account, as was shown [here](#Importing-Australian-Financial-to-Rguroo).
...
6.  The economic crisis of 2008 affected many countries, though some more than others. Some people in Australia have claimed that Australia wasn't hurt that badly from the crisis. The forst 6 rows for the dataset containing bank assets (in billions of Australia dollars (AUD)) of the Reserve Bank of Australia (RBA) for the time period of September 1 1969, through March 1 2019, are contained in @bl-Australian (Reserve Bank of Australia, 2019). Create a time-series plot of the assets of the RBA and interpret any findings.
