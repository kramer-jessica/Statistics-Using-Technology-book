...
For a proportion, the natural starting point is a **count**. If we sample $n$ individuals and record how many fall into the category of interest, that count — written $X$ — is itself a statistic, and dividing it by the sample size gives the **sample proportion** $\hat{p} = X/n$, our estimate of $p$. Both $X$ and $\hat{p}$ carry the same information, but it will be convenient to work with the count $X$ when we describe the sampling distribution in the proportion chapter, because $X$ is the quantity whose distribution we know directly. For a mean, the statistic is the **sample mean** $\bar{X}$, our estimate of $\mu$.
...
Throughout these chapters, we use different notation for a statistic before and after sampling. Before the sample is collected, a statistic is a random variable because its value depends on which sample happens to be selected. We represent these random statistics with capital letters or hats: $X$, $\hat{p}$, and $\bar{X}$. After the sample is collected, the statistic takes on one specific observed value. We represent observed values with lowercase letters and the subscript “obs”: $x_{\text{obs}}$, $\hat{p}_{\text{obs}}$, and $\bar{x}_{\text{obs}}$.
...
The form of the hypotheses is dictated by the research question, not by the data. The phrase "more than," "increased," or "exceeds" signals a Case 1 alternative; "less than," "fewer," or "decreased" signals Case 2; and "differs from," "changed," or "is not equal to" signals a two-sided Case 3 alternative. For instance, asking whether *more than* 70% of students use YouTube daily, or whether mean math SAT scores are *higher than* the national mean of 508, gives a Case 1 alternative; asking whether *fewer than* 35% of adults are physically inactive, or whether mean nightly sleep is *less than* 7 hours, gives a Case 2 alternative; asking whether the proportion getting their news from social media *differs from* 36%, or whether mean commute time *differs from* the national figure, gives a two-sided Case 3 alternative.
...
The following examples show the same three forms arising in both the proportion and the mean settings.
...
@fig-pvalue-youtube shows this p-value as the probability that the count $X$ is at least as large as the observed $x_{\text{obs}} = 17$, computed from the sampling distribution of $X$ when $p = 0.70$.

![The p-value for the YouTube test, viewed through the count statistic $X$ in a sample of $n = 20$. The bars give the sampling distribution of $X$ when the boundary value $p_0 = 0.70$ holds. The observed count is $x_{\text{obs}} = 17$, and the p-value is the total probability of the red-colored outcomes $X \geq 17$ that are at least as supportive of $H_a: p > 0.70$.](Rguroo_outputs/Foundations_of_Inference/pvalue-youtube.png){#fig-pvalue-youtube width="85%"}
:::
