...
Sampling variability is what makes inference more complicated than simply comparing a sample statistic to an assumed population value. If a sample statistic differs from an assumed population value, we cannot immediately conclude that the claim is wrong, because some difference is expected purely by chance. Even if the the value we believe is true, different random samples will naturally produce somewhat different statistics. The entire apparatus of inference is built to answer one question: *is the difference we observe larger than what sampling variability alone would reasonably produce by chance?* The probability distributions from earlier chapters give us a way to answer this question, because they describe how a statistic behaves across all possible samples.
...
Now suppose we wish to know whether the mean math SAT score of incoming freshmen at a university is *higher than* the national mean of 508 points. A random sample of freshmen gives an observed sample mean of $\bar{x}_{\text{obs}} = 542$ points. Again the observed value points in the direction of the claim — 542 is above 508 — but we should ask whether a sample mean this large could plausibly occur even if the true mean were 508 or less.

::: {.callout-note}
## Consistent With a Claim Is Not the Same as Strong Evidence

A sample result that points in the direction of a claim does not by itself establish the claim. Because of _sampling variability_, such a result can _occur by chance_ even when the claim is false. The goal of statistical inference is to determine whether the observed result is unlikely to have occurred by chance alone.
:::

The reasoning behind a hypothesis test follows the same general steps whether we are testing a population parameter, a population mean, or any other population parameter:

1. State two competing claims about the population parameter: the **null hypothesis** and the **alternative hypothesis**.
2. Assume, for the sake of argument, that **the null hypothesis is true**, and determine how the sample statistic would behave from sample to sample under that assumption.
3. Compare the observed sample statistic to this expected behavior. If the observed value would be unlikely when the null hypothesis is true, take this as evidence against the null hypothesis.
4. Reach a conclusion (decision): either **reject** the null hypothesis in favor of the alternative hypothesis, or **fail to reject** the null hypothesis.
...
The claim we are trying to find evidence *for* — the new effect, the difference, the departure from the status quo — is called the **research hypothesis**, more commonly the **alternative hypothesis**, denoted $H_a$. It is the statement the investigator suspects may be true and hopes to support with data.
...
Whatever the parameter, the pair of hypotheses always takes one of three forms, depending on the direction of the research question. We denote the value that separates the null hypothesisfrom the alternative hypothesis by $p_0$ for a population proportion and $\mu_0$ for a population mean. We call this value the **boundary value** (sometimes the *hypothesized value*). 

The three possible forms are the same for proportions and means; only the symbol for the parameter changes:
...
In Cases 1 and 2 the alternative hypothesis specifies a **direction**. It claims that the population parameter is either **greater than** $(>)$ or **less than** $(<)$ the boundary value. These are **one-sided** (or **one-tailed**) hypotheses. Specifically,  Case 1 is a  **right-tailed**, and Case 2 is a **left-tailed** hypothesis. 

In Case 3 the alternative hypothesis states only that the parameter is **different from** $(\neq)$ from the boundary value. It does not specify whether the parameter is smaller or larger. This is called a **two-sided** (or **two-tailed**) hypothesis.
...
::: {#exm-right-tailed-proportion}
## Right-Tailed Test for a Population Proportion

A recent survey found that approximately **70%** of college students use YouTube every day. A researcher believes that the percentage at her university is **higher** than 70%.

Identify the population parameter and write the null and alternative hypotheses.
...
$p=$ the proportion of all students at the university who use YouTube every day.
...
The alternative hypothesis represents the researcher's claim that the proportion exceeds 70%.

::::

---

::: {#exm-left-tailed-proportion}
## Left-Tailed Test for a Population Proportion

Health officials report that **35%** of adults are physically inactive. A public health researcher wants to determine whether the proportion in her community is **lower** than 35%.
...
$p=$ the proportion of all adults in the community who are physically inactive.
...
The alternative hypothesis represents the researcher's claim that the community has a lower rate of physical inactivity.

::::

---

::: {#exm-two-sided-proportion}
## Two-Sided Test for a Population Proportion

A national survey reports that **36%** of adults obtain most of their news from social media. A researcher wants to determine whether the proportion in her state is **different** from 36%.
...
$p=$ the proportion of all adults in the state who obtain most of their news from social media.
...
The alternative hypothesis represents the researcher's claim that the proportion differs from the national value.

::::

---

::: {#exm-right-tailed-mean}
## Right-Tailed Test for a Population Mean

The national average Math SAT score is **508**. A high school principal believes that students at her school have a **higher** average Math SAT score.
...
$\mu=$ the mean Math SAT score of all students at the high school.
...
The alternative hypothesis represents the principal's belief that the school's average exceeds the national average.

::::

---

::: {#exm-left-tailed-mean}
## Left-Tailed Test for a Population Mean

Sleep experts recommend about **7 hours** of sleep per night for adults. A researcher suspects that adults in a particular city sleep **less** than 7 hours per night, on average.
...
$\mu=$ the mean number of hours of sleep per night for all adults in the city.
...
The alternative hypothesis represents the researcher's claim that the mean nightly sleep is less than 7 hours.
...
::: {#exm-two-sided-mean}
## Two-Sided Test for a Population Mean

The national average one-way commute time is **27 minutes**. A transportation planner wants to determine whether the mean commute time in her city is **different** from the national average.
...
$\mu=$ the mean one-way commute time for all workers in the city.
...
The alternative hypothesis represents the planner's claim that the city's mean commute time differs from the national average.

::::

<!---
For instance, asking whether *more than* 70% of students use YouTube daily, or whether mean math SAT scores are *higher than* the national mean of 508, gives a Case 1 alternative; asking whether *fewer than* 35% of adults are physically inactive, or whether mean nightly sleep is *less than* 7 hours, gives a Case 2 alternative; asking whether the proportion getting their news from social media *differs from* 36%, or whether mean commute time *differs from* the national figure, gives a two-sided Case 3 alternative.
-->

### Why the Boundary Value Matters {#sec-foundations-boundary}

As noted earlier, a hypothesis test begins by assuming the null hypothesis is true and asking how the sample statistic would behave under that assumption. For this we need to work with one specific value of the parameter. In the two-sided Case 3 the answer is immediate: the null hypothesis $H_0: p = p_0$ (or $H_0: \mu = \mu_0$) names a single value, so "$H_0$ is true" means the parameter equals that one value, so there is no question about which value to use. In the one-sided Cases 1 and 2, however, the null hypothesis is not a single value but a whole range — for example, $H_0: p \leq 0.70$ allows any proportion at or below 0.70. In principle, we can assume any value in this range as the true population proportion. Fortunately, there is a natural choice that serves as the basis for the hypothesis test. So what is this choice?

The choice is the **boundary value** — the value $p_0$ or $\mu_0$ that separates the null hypothesis from the alternative hypothesis. The reason comes from the need to control errors in hypothesis testing, which we discuss in @sec-foundations-errors. Every hypothesis test must keep the probability of incorrectly rejecting the null hypothesis $H_0$ at or below a chosen level. To do this, we use the value of the parameter that is hardest to distinguish from the alternative hypothesis. That value is the boundary value. A claim like $p \leq 0.70$ is most easily mistaken for the alternative "$p > 0.70$" when the truth sits right at $0.70$; a true proportion well below 0.70 would rarely provide evidence suggesting $p > 0.70$. Using the boundary value therefore gives the most **conservative hypothesis test**: if the error rate is controlled at the boundary, it is automatically controlled for every other value in the null region. You will see a more detailed explanation of why the maximum error probability occurs at the boundary in @sec-boundary-value in the next chapter.
...
## Making a Decision in a Hypothesis Test {#sec-foundations-decision}

Once we have stated the null and alternative hypotheses, the next step is to determine whether the observed data provide sufficient evidence against the null hypothesis and in favor of the alternative hypothesis. Although six logical combinations of decisions and hypotheses can be imagined, a hypothesis test leads to only two conclusions: either we **reject $H_0$ in favor of $H_a$** or we **fail to reject $H_0$**. These possibilities are summarized in @fig-hypothesis_decision_matrix.

![The six logical combinations of decisions and hypotheses.](Rguroo_outputs/Foundations_of_Inference/hypothesis_decision_matrix.png){#fig-hypothesis_decision_matrix width="90%"}

First, let's clarify the distinction between **accepting** $H_0$ and **failing to reject** $H_0$. This distinction is easier to understand by considering a criminal trial. In our judicial system, a defendant is presumed innocent unless proven guilty beyond a reasonable doubt. The hypotheses can therefore be written as
...
If there is not sufficient evidence to conclude that the defendant is guilty beyond a reasonable doubt, we **fail to reject** $H_0$; that is, we fail to reject the claim that the defendant is innocent. However, this does not mean that we have proved the defendant is innocent. A verdict of *not guilty* simply means that the evidence was not strong enough to establish guilt. The trial is designed to look for convincing evidence of guilt, not to prove innocence.

Hypothesis tests follow the same logic. They are designed to look for evidence against the null hypothesis and in favor of the alternative hypothesis, not to prove that the null hypothesis is true. If the observed data provide sufficiently strong evidence against $H_0$, we **reject $H_0$ in favor of $H_a$**. Otherwise, we **fail to reject $H_0$**.

Although our goal is to find evidence in favor of the alternative hypothesis, we do so by evaluating the null hypothesis. We begin by assuming $H_0$ is true and ask whether the observed data are inconsistent with that assumption. Consequently, the conclusion of a hypothesis test is always expressed in terms of the null hypothesis: we either **reject $H_0$** or **fail to reject $H_0$**.


### The Two Types of Error {#sec-foundations-errors}

Because a hypothesis test reaches a decision from a sample rather than from the whole population, our conclusion can be wrong. There are exactly two ways to be wrong, and distinguishing them is essential to understanding how tests are designed.
...
@fig-types-of-error traces the same four outcomes as a flow from the unknown truth, through our decision, to the result. The truth about $H_0$ is unknown to us (left). For each possibility we may either reject or fail to reject $H_0$ (middle), leading to a correct decision or to one of the two errors (right): a Type I error rejects a true $H_0$, and a Type II error fails to reject when $H_a$ is true.

![The four possible outcomes of a hypothesis test.](Rguroo_outputs/Foundations_of_Inference/types_of_error.png){#fig-types-of-error width="90%"}

We measure these errors using probabilities. Specifically, $\alpha$ (the Greek letter alpha) denotes the probability of a Type I error, and $\beta$ (the Greek letter beta) denotes the probability of a Type II error.
...
We would like to make both kinds of error, Type I and Type II, as rarely as possible. Unfortunately, for a fixed sample size, the two are connected: reducing the chance of one error increases the chance of the other.

Let's look at a smoke detector example to clarify this trade-off. Let $H_0$ be "there is no fire" and $H_a$ be "there is a fire." If we make the detector less sensitive to avoid false alarms from burnt toast or steam, we reduce Type I errors but increase the risk that the detector stays silent during a real fire, creating more Type II errors. If we make the detector more sensitive so it rarely misses a real fire, we reduce Type II errors but increase false alarms, creating more Type I errors.
The same trade-off appears in every hypothesis test. Making it harder to reject $H_0$ reduces the chance of a Type I error but increases the chance of a Type II error. Making it easier to reject $H_0$ has the opposite effect. For a fixed sample size, the two types of errors cannot both be minimized at the same time—every hypothesis test must strike a balance between them.

![The trade-off between Type I and Type II errors.](Rguroo_outputs/Foundations_of_Inference/tradeoff-seesaw.png){#fig-tradeoff-seesaw width="80%"}


Faced with this trade-off, statisticians choose to focus first on controlling Type I errors: *we keep the maximum probability of a Type I error at a small prespecified level called **significance level**, and then try to make the probability of a Type II error as small as possible within that constraint.* One reason for focusing on Type I errors is that falsely claiming an effect exists — for example, saying a new drug works when it does not — can have serious consequences. 
...
To make the idea of "consistent" more precise, we tentatively assume that $H_0$ is true. This means assuming the boundary value of the null hypothesis: $p = p_0$ for a proportion or $\mu = \mu_0$ for a mean. Under this assumption, each of the statistics — the count $X$, the sample proportion $\hat{p}$, and the sample mean $\bar{X}$ — has a known sampling distribution describing how it varies from sample to sample. Using this distribution, we then ask the central question: assuming $H_0$ is true, what is the probability that the statistic ($X$, $\hat{p}$, or $\bar{X}$) takes a value at least as far in favor of $H_a$ as the value we actually observed ($x_{\text{obs}}$, $\hat{p}_{\text{obs}}$, or $\bar{x}_{\text{obs}}$)?

Intuitively, the p-value tells us how surprising the data we observed would be if the null hypothesis were actually true. Even when $H_0$ holds exactly, we don't expect a sample statistic to land precisely on $p_0$ or $\mu_0$ every time — sample-to-sample variation is normal. The p-value asks a sharper question: among all the outcomes that could occur by random sampling under $H_0$, how extreme is the one we actually got? A value near 0 says our data would be a rare, surprising result if $H_0$ were true; a value near 1 says our data look like an unremarkable draw from a world where $H_0$ holds. The box below states this precisely.

::: {.callout-note}
## The p-Value

The **p-value** is the probability, computed *assuming the null hypothesis is true*, of obtaining a value of the statistic at least as **favorable to** the alternative hypothesis $H_a$ as the observed value.

A small p-value means the observed data are inconsistent with $H_0$, and therefore count as evidence *against* $H_0$. A large p-value means the data are consistent with $H_0$, and therefore do not provide strong evidence against it.
:::
...
What counts as "at least as supportive of $H_a$" depends on the direction of the alternative. We call the collection of such values the **$H_a$ support**, and the p-value is the probability of the statistic landing in this region when $H_0$ is true. The crucial point is that the $H_a$ support is anchored at the *observed value* — it is defined by which outcomes are as convincing as, or more convincing than, what we actually observe. We illustrate all three cases with the SAT example, where the boundary value is $\mu_0 = 508$.
...
Once the p-value is in hand, the decision rule is the same in every case and compares the p-value to the significance level $\alpha$, chosen in advance.
...
The logic is direct. If the observed result, and results even more extreme, would be unlikely to occur under $H_0$ — unlikely enough that their combined probability falls at or below $\alpha$ — we treat that as sufficient evidence to reject $H_0$. If instead the result is the sort of thing that happens fairly often under $H_0$, we have no strong reason to reject the null. @fig-pvalue-decision summarizes this rule.

![The p-value decision rule.](Rguroo_outputs/Foundations_of_Inference/pvalue-decision-rule.svg){#fig-pvalue-decision width="90%"}

A useful feature of the p-value is that it conveys not just a yes-or-no decision but the *strength* of the evidence. A p-value far below the significance level $\alpha$ indicates very strong evidence against $H_0$; a p-value just barely below $\alpha$ indicates evidence that clears the bar only narrowly.
...
A hypothesis test answers a yes-or-no question about a *single* candidate value of the parameter: is that value (call it the boundary value, $p_0$ or $\mu_0$) consistent with the data, or not? Often, though, we want something more informative — not a verdict on one value, but a whole range of values that the data find plausible. This is the role of a **confidence interval**. The two tools answer different questions, but as we will see, they are built from exactly the same sampling distributions.

Before explaining how confidence intervals are constructed, let's first see what one looks like and how to interpret it. Suppose we survey $n = 400$ registered voters and find that $\hat{p}_{\text{obs}} = 0.55$ support a proposed policy. Rather than testing a single hypothesized value of $p$, we might instead ask: what range of values of $p$ are plausible given our data? A method we will construct later in this section might give us the interval $(0.50, 0.60)$, along with a **confidence level** of 95%. We would report this as: "we are 95% confident that the true proportion of voters who support the policy is between 0.50 and 0.60."

The same reasoning applies to a population mean. Instead of estimating a population proportion, suppose a sample of $n = 50$ employees has an average commute time of $\bar{x}_{\text{obs}} = 27.3$ minutes. A 95% confidence interval for the true mean commute time $\mu$ might be $(25.1, 29.5)$ minutes — we are 95% confident that the true average commute time falls in this range.

Both examples report a 95% confidence interval. But what does "95% confident" actually mean here? The answer is subtler than it first appears, and we will make it precise shortly. For now, notice something familiar in both examples: each interval consists of the observed statistic ($\hat{p}_{\text{obs}} = 0.55$, $\bar{x}_{\text{obs}} = 27.3$) and extends some distance in each direction — a structure that will turn out to be closely connected to the hypothesis tests.

At first glance, confidence intervals and hypothesis tests may seem like different inferential tools. In reality, they are two sides of the same idea. The key realization is that we already possess everything needed to build such a range. A two-sided hypothesis test is, in effect, a *consistency check* for a proposed value: test the parameter against that single value, and if we fail to reject, the value is consistent with our data; if we reject, it is not. Nothing stops us from running that check not for one candidate value but for *every* candidate value. Collect all the values that survive — all the values we would fail to reject — and the resulting set is a confidence interval. This connection can be summarized in a simple rule, called **inverting a test**.
...
To picture the inversion, imagine sweeping a candidate value along the number line and testing each one, as in @fig-ci-inversion. Consider the SAT example, where a sample of math SAT scores has an observed mean of $\bar{x}_{\text{obs}} = 542$. Each dot marks a candidate value for the population mean $\mu$. A two-sided test rejects candidate values far from 542 (coral) and fails to reject those close to it (teal). The stretch of non-rejected values between the two switching points — marked by the bracket — is the confidence interval: the band of mean values the data deem plausible. (No computation is shown here; the figure simply records, for a grid of candidate values, which ones a test would reject.)

![A confidence interval as an inverted test, illustrated for the mean math SAT score.](Rguroo_outputs/Foundations_of_Inference/ci-by-inversion-mean.svg){#fig-ci-inversion width="100%"}

This construction also explains what the confidence level *means* — and the idea is easy to misstate, so it's worth stating carefully.
...
Confidence intervals and hypothesis tests answer different questions but are built from the same sampling distributions. A hypothesis test asks whether one particular value is plausible, whereas a confidence interval identifies all values that are plausible.

Before explaining how confidence intervals are constructed, let's first see what one looks like and how to interpret it. Suppose we survey $n = 400$ registered voters and find that $\hat{p}_{\text{obs}} = 0.55$ support a proposed policy. Rather than testing a single hypothesized value of $p$, we might instead ask: what range of values of $p$ are plausible given our data? A calculation we will develop in the next chapter might give us the interval $(0.50, 0.60)$, along with a **confidence level** of 95%. We would report this as: "we are 95% confident that the true proportion of voters who support the policy is between 0.50 and 0.60."

The same reasoning applies to a population mean. Instead of estimating a population proportion, suppose a sample of $n=50$ employees has an average commute time of $\bar{x}_{\text{obs}} = 27.3$ minutes. A 95% confidence interval for the true mean commute time $\mu$ might be $(25.1, 29.5)$ minutes — we are 95% confident that the true average commute time falls in this range.

Both examples report a 95% confidence interval, but what does the phrase "95% confident" actually mean? The answer is subtle. The confidence level does not describe our certainty about this particular interval. Instead, it describes the reliability of the method used to construct the interval.
If we repeated the sampling process many times and built an interval the same way each time, about 95% of those intervals would capture the true parameter. We will make this idea precise shortly. For now, notice something familiar in both examples: each interval consists of the observed statistic ($\hat{p}_{\text{obs}} = 0.55$, $\bar{x}_{\text{obs}} = 27.3$) and extends some distance in each direction — a structure that will turn out to be closely connected to the hypothesis tests.

At first glance, confidence intervals and hypothesis tests may seem like different inferential tools. In reality, they are two sides of the same idea. The key realization is that we already possess everything needed to build such a range. A two-sided hypothesis test is, in effect, a *consistency check* for a proposed value: test the parameter against that single value, and if we fail to reject, the value is consistent with our data; if we reject, it is not. Nothing stops us from running that check not for one candidate value but for *every* candidate value. Collect all the values that survive — all the values we would fail to reject — and the resulting set is a confidence interval. 
This connection can be summarized in a simple rule, called **inverting a test**.
