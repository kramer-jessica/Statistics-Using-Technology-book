...
The dataset [beer_data]{.data} contains information on 227 different 12-ounce beers, including alcohol content and number of calories for each beer. The data were obtained from the [Beer100](https://www.beer100.com/beer-calories/){target=”_blank” rel=“noopener”} website.

a. Using the data in [beer_data]{.data}, construct a scatterplot with alcohol content as the predictor variable and number of calories as the response variable.

b.  Based on your plot, describe the relationship between alcohol content and calories. Do the points appear to follow a linear pattern? Explain.
...
Table @tbl-Beer displays the first six rows of the dataset. A complete description of the variables is provided in the  [dataset code book](#code-book-beer) that follows.
...
**Description** This dataset contains a collection of popular beers from large breweries. For each beer, the data include the number of calories, grams of carbohydrates, and alcohol content for a 12-ounce serving. The collection includes both domestic and imported beers. For imported beers, the nutritional information has been standardized to a 12-ounce serving, even though many imported beers are typically sold in pint-sized containers.

This dataset contains the following variables (columns):
...
## 🔎 Solution
(a) To aid in figuring out if there is a relationship between the alcohol content and the number of calories, we draw a scatterplot of the data. First, it is helpful to clearly state the variables. Following the convention of algebra, we represent the predictor variable as $x$ and the response variable as $y$. In this example, $x$ = percent alcohol content in the beer; [alcohol]{.var}, and $y$ = number of calories in 12 ounce beer; [calories]{.var}
...
b. The scatterplot, displayed in @fig-Beer-point, shows a clear upward trend in the data. As alcohol content increases, the number of calories also tends to increase. The points cluster around what appears to be a straight-line pattern, even though they are not perfectly aligned.

This overall pattern suggests that a straight line would do a reasonable job of describing how calories change as alcohol content changes.

:::

### Finding the Regression Line

To describe the linear relationship between two quantitative variables, we use a statistical method called **regression**. This method finds the straight line that best fits the data points on a scatterplot. The resulting line is called the **least squares line**. It is also commonly referred to as the **regression line**, and in this text we will use both terms. The equation of this line summarizes how the response variable changes as the predictor variable changes.

To determine the least squares line, we imagine drawing a line through the data and measuring the vertical distance from each point to the line. These vertical distances are called **residuals**. The least squares line is the line that makes the sum of the squares of these residuals as small as possible. @fig-Beer-point-line displays this line on the scatterplot of Number of Calories versus Alcohol Content.
...
$$\text{residual} = y - \hat{y} = \mbox{observed value} − \mbox{predicted value}.$$
...
In this section, we learn how to find the equation of the regression line and use it to make predictions. The regression equation describes the relationship between the predictor variable and the response variable, allowing us to estimate the value of the response for a given value of the predictor variable.

::: {#exm-find-regression-equation}
## Find the Equation of the Regression Line

Is there a relationship between the alcohol content and the number of calories in 12-ounce beer? To determine if there is one, we examine the sample of beer's alcohol content and calories, a portion of which is shown in @tbl-Beer.
...
The table provides summary information about the regression analysis. In particular, the last row, labeled "Equation of Least Squares Line," shows the equation of the least squares line, including the estimated slope and $y$-intercept. In the displayed equation, calories is written in place of $y$ and alcohol in place of $x$.
...
To make a prediction, we substitute the given alcohol content (as a percent) into the regression equation and compute the corresponding fitted value. In this example, we want to predict the number of calories for a beer with 7% alcohol content. Therefore, we substitute 7 in for $x$ and perform the following calculation:
$$
\hat{y} =  14.52 + 26.72 \times 7  = 201.56.
$$

The value, 201.56, is the predicted number of calories for a beer whose alcohol content is 7\%. So, if you are drinking a beer that has 7\% alcohol content, then it probably has close to 202 calories.
...
Again, to predict the number of calories for a beer with alcohol content of 14%, we substitute 14 in for $x$ in the regression equation.
...
Notice that 7% alcohol falls within the values of alcohol content observed in the data. Predicting a value for an $x$-value inside the observed values is called **interpolation**. When you interpolate, you can usually feel confident that the predicted value will be close to the true value because the model was built using data from this set of alcohol content values and is meant to describe relationships for alcohol levels like these. When you extrapolate, however, you cannot be sure the prediction is close to the true value, because the relationship may change for $x$-values beyond those observed when the model was created.
...
However, using the same model to predict height at age 40 represents an
...
Clearly a height of 130 inches (10 feet and 10 inches) is an unrealistic value. This illustrates why extrapolation can lead
...
As we will see in @sec-assumption-validity-line, because residuals represent the unexplained variation in the response variable, examining them carefully helps us determine whether a linear regression model is appropriate.
...
### Checking Validity of the Regression Equation {#sec-assumption-validity-line}
...
The assumption of linearity means that the relationship between the predictor variable and the response variable can be reasonably described by a straight line. In particular, for any given $x$-value, the average (mean) of the corresponding $y$-values should lie on, or very close to, the least squares regression line. This does not mean that all points fall on the line, but that the overall trend in the data follows a linear pattern.
...
@fig-Beer-resvsfit shows the residual-versus-fit plot for the beer data, which is also part of the [Linear Regression]{.fun} function output in Rguroo. Each point represents one beer: its horizontal position is the predicted number of calories, and its vertical position is the residual (observed calories minus predicted calories). The points appear to be randomly scattered around the horizontal line at zero. The green curve in the plot is a smooth curve that follows the overall pattern of the residuals. The curve is mostly flat, although it bends slightly near the ends due to a few observations with larger residuals. Small deviations from a flat line that are caused by a few mild outliers are typically not a serious concern. There is no clear overall curved pattern in the plot, suggesting that the linearity condition is reasonably satisfied.
...
In some situations, the relationship between a predictor variable and a response variable may differ across categories of a third variable. When we fit and compare regression models separately for different groups as separated by a categorical variable, this is called regression by group. This approach is useful when we want to determine whether the relationship between $x$ and $y$ differs depending on group membership—for example, whether the slope, intercept, or overall pattern differs across categories.
...
Click to expand the box below to see how to create a scatterplot of calories vs. alcohol content separated by the factor [location]{.var}.
...
We can also obtain the regression equation for each level of a factor separately in Rguroo. The quickest way to display these equations is to select the checkbox [Show Correlation and LS Equation]{.des} in the [Scatterplot]{.dialog} dialog, as shown in @fig-scatterplot_dialog_LS_factor.
...
$$
\begin{aligned}
\text{\bf Least Squares Lines:}&\\
\text{Domestic:} \quad \text{calories} &= 9.4053 + 27.505 \cdot \text{alcohol} \\
\text{Import:} \quad \text{calories} &= 32.524 + 23.763 \cdot \text{alcohol}
\end{aligned}
$$
...
1.  When an anthropologist finds skeletal remains, one task is to estimate the person’s height. Data on height (in cm) and the length of metacarpal bone 1 (in mm) were collected and are shown in @tbl-Metacarpal (“Prediction of Height,” 2013).
    ...
    a. Create a scatterplot with metacarpal length as the predictor and height as the response. Then draw the least squares regression line on the same graph so that you can see how well the line fits the data. Does the pattern look approximately linear.
...
2. @tbl-House contains a portion of the dataset [house]{.data} that contains data on house value and the annual rental income each house generates (“Capital and Rental,” 2013).
    ...
    a. Create a scatterplot of rental income versus house value, using house value as the predictor variable and annual rental income as the response variable. Label both variables clearly and determine whether the points appear to follow a linear pattern.
    ...
    g. Interpret the $y$-intercept in context. Is the $y$-intercept meaningful in this situation? Explain.
...
3. @tbl-Fertility contains a portion of the dataset [Life_exp_fert_rate]{.data} which contains data from 2011 on fertility rate (births per woman) and life expectancy (in years) for various countries (“Life Expectancy At,” 2013; “Fertility Rate,” 2013).
    ...
    a. Create a scatterplot of life expectancy versus fertility rate, using fertility rate as the predictor variable and life expectancy as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.
...
4. The World Bank collected data on the percentage of gross domestic product (GDP) that a country spends on health expenditures (Current health expenditure (% of GDP), 2019), the fertility rate of the country (Fertility rate, total (births per woman), 2019), and the percentage of women receiving prenatal care (Pregnant women receiving prenatal care (%), 2019). The dataset for the countries where this information is available is [fertility_prenatal]{.data}, part of which is shown in @tbl-Fert_prenatal.
    ...
    a. Create a scatterplot of the percentage of women receiving prenatal care versus the percentage of GDP spent on health expenditures, using health expenditure percentage as the predictor variable and prenatal care percentage as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.
...
5. The height and weight of baseball players are given in [baseball]{.data} dataset, part of which is shown in @tbl-Baseball ("MLB heightsweights," 2013).
    ...
    a. Create a scatterplot of weight versus height, using height (in inches) as the predictor variable and weight (in pounds) as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.
...
[Player]{.var}: Player in the sample

[height]{.var}: height of baseball player (inches)

[weight]{.var}: weight of baseball player (pounds)
...
6. Data on body weights and brain weights are given in the dataset [body]{.data}, part of which is shown in @tbl-Body ("Brain2bodyweight," 2013).
    ...
    a. Create a scatterplot of brain weight versus body weight, using body weight (in kg) as the predictor variable and brain weight (in g) as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.
...
[species]{.var}: species of animal

[bodyweight]{.var}: the body weight of the species (kg)

[brainweight]{.var}: the brain weight of the species (kg)

[brainbodyproportion]{.var}: the ratio of brain weight to body weight of the species
...
7. A sample of hot dogs was taken and the amount of sodium (in mg) and calories were measured and recorded in the dataset [hotdog]{.data} ("Data hotdogs," 2013). The first six rows of the dataset are shown in @tbl-Hotdog.
    ...
    a. Create a scatterplot of sodium versus calories, using calories as the predictor variable and sodium as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.
    ...
    g. What is the $y$-intercept? Interpret the $y$-intercept in the context of the problem. Is your interpretation practically meaningful?
...
[type]{.var}: categorical variable indicating the type of hot dog. Typical categories include Beef, Meat, and Poultry.

[calories]{.var}: quantitative variable giving the number of calories per hot dog.

[sodium]{.var}: quantitative variable giving the sodium content per hot dog, measured in milligrams.
...
8. Per capita income in 1960 dollars for European countries and the percent of the labor force that works in agriculture in 1960 are given in the dataset [agriculture]{.data} ("OECD economic development," 2013). The first six rows of this dataset are shown in @tbl-Agriculture.
    ...
    a. Create a scatterplot of per capita income versus percent of labor force in agriculture, using percent of labor force in agriculture as the predictor variable and per capita income (in 1960 dollars) as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.
...
[country]{.var}: country in Europe

[percapita]{.var}: per captia income

[agriculture:]{.var} percentage of workforce in agriculture

[industry]{.var}: percentage of workforce in industry

[services]{.var}: percentage of workforce in services

[Source](http://lib.stat.cmu.edu/DASL/Datafiles/oecdat.html) OECD economic development. (2013, December 04).

Reference: Data And Story Library
...
9. Cigarette smoking and cancer have been linked. The number of deaths per one hundred thousand from bladder cancer and the number of cigarettes sold per capita in 1960 are given in the dataset [cancer_1]{.data} ("Smoking and cancer," 2013). The first six rows of the data are shown in @tbl-Cancerbladder.
    ...
    a. Create a scatterplot of deaths from bladder cancer versus number of cigarettes sold per capita, using number of cigarettes sold per capita as the predictor variable and number of deaths per one hundred thousand from bladder cancer as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.

    b. Determine the least squares regression equation relating the number of deaths from bladder cancer to the number of cigarettes sold per capita. Write the equation in the form $\hat y = b + mx$.
...
[state]{.var}: state in US

[cig]{.var}: the number of cigarette sales (per capita)

[bladder]{.var}: number of deaths per 100 thousand from bladder cancer

[lung]{.var}: number of deaths per 100 thousand from lung cancer

[kidney]{.var}: number of deaths per 100 thousand from kidney cancer

[leukemia]{.var}: number of deaths per 100 thousand from leukemia
...
10. The weight of a car can influence the mileage that the car can obtain. A random sample of cars' weights and mileage was collected and reported in the dataset [cars]{.data}, with the first six rows shown in @tbl-Cars ("us auto mileage," 2019).
    ...
    a. Create a scatterplot of mileage versus car weight, using car weight (in pounds) as the predictor variable and mileage (in miles per gallon) as the response variable. Label both variables clearly, and determine whether the points appear to follow a linear pattern.
...
[make]{.var}: the type of car

[vol]{.var}: cubic feet of cab space

[hp]{.var}: engine horsepower

[npg]{.var}: the average mileage of the car

[sp]{.var}: top speed (mph)

[wt]{.var}: the weight of the car (100 pounds)
...
@fig-correlation-image shows six scatterplots illustrating different values of the correlation coefficient $r$. The plots demonstrate how the sign of $r$ indicates the direction of the linear relationship, while the magnitude of $r$ reflects the strength of that relationship, ranging from no linear pattern to perfect positive or negative linear relationships.



```{r correlation_images, echo=FALSE}
#| fig-alt: "Six scatterplots arranged in a 2×3 grid labeled (a) through (f), illustrating different values of the correlation coefficient r. (a) Perfect positive correlation (r = 1): all points lie exactly on an upward-sloping straight line. (b) Strong positive correlation (r = 0.8): points cluster closely around an upward-sloping line. (c) No correlation (r = 0): points are randomly scattered with no clear linear pattern. (d) Weak negative correlation (r = -0.1): points show a very slight downward tendency with substantial scatter. (e) Strong negative correlation (r = -0.8): points cluster closely around a downward-sloping line. (f) Perfect negative correlation (r = -1): all points lie exactly on a downward-sloping straight line. Each subplot displays the value of r inside the graph area."
#| warning: FALSE
#| label: fig-correlation-image
#| fig-cap: "Scatterplots illustrating different directions and strengths of linear relationships for selected values of the correlation coefficient $r$."
#| out-width: "90%"
knitr::include_graphics("Rguroo_outputs/Regression/correlation_images.png")
```

In plot (a), $r = 1$, and all points lie exactly on an upward-sloping straight line, representing a perfect positive linear relationship. In plot (b), $r = 0.8$, the points cluster closely around an upward-sloping line, indicating a strong positive linear relationship. In plot (c), $r = 0$, the points are scattered without any clear linear pattern, indicating no linear relationship. In plot (d), $r = -0.1$, the points show a very slight downward tendency with substantial scatter, representing a weak negative linear relationship. In plot (e), $r = -0.8$, the points cluster closely around a downward-sloping line, indicating a strong negative linear relationship. In plot (f), $r = -1$, all points lie exactly on a downward-sloping straight line, representing a perfect negative linear relationship.
...
How strong is the linear relationship between alcohol content and the number of calories in a 12-ounce beer? To answer this question, use Rguroo and the [beer]{.data} dataset to calculate the correlation coefficient between alcohol content and calories. The first six rows of this dataset are shown in @tbl-Beer. Interpret the value of $r$ in context.
:::
...
One common mistake is to assume that if two variables are correlated, then one causes the other. This is not necessarily true. For example, it would be incorrect to say that the alcohol content of a beer causes it to have a certain number of calories. In reality, the amount of sugar used during fermentation affects both the alcohol content and the number of calories. In this case, sugar is a third variable that influences both variables we observe. A variable like this is called a **confounding** variable. Confounding occurs when a third variable influences both of the variables being studied, making it appear that there is a direct relationship between them.
...
The following three examples are illustrations of confounding variables.
...
Data show a positive correlation between ice cream sales and drowning incidents. Does eating ice cream cause drowning?

It is unlikely that ice cream consumption directly causes drowning. Both ice cream sales and drowning incidents tend to increase during the summer months. Warmer weather is a third variable that increases swimming activity and ice cream consumption at the same time. In this case, temperature explains the relationship between the two variables.

**Example 3**: Firefighters and fire damage  
Data show a strong correlation between the number of firefighters at a fire and the amount of damage caused. Does the presence of more firefighters cause greater damage?

It would be incorrect to conclude that firefighters cause more damage. Larger fires require more firefighters and also result in greater damage. The size of the fire is a third variable that explains both the number of firefighters and the amount of damage.
...
**For each problem, state the random variables.The datasets in this section are in section 10.1 and will be used in section 10.3.**
...
1. When an anthropologist finds skeletal remains, they often estimate the height of the person using measurements of certain bones. The height of a person (in cm) and the length of their first metacarpal bone (in mm) were collected. The data are in the [height_vs_metacarp]{.data} dataset, and the first six rows are shown in @tbl-Metacarpal ("Prediction of height," 2013).
    ...
    c. Based on the value of $r$, would you describe the linear relationship as strong, moderate, or weak? Explain briefly.
...
The codebook for the dataset [cancer](#code-book-cancer) is given below @tbl-Cancerbladder.
...
11.	A researcher measures the effect of fertilizer on plant growth by randomly assigning identical plants to receive either no fertilizer or a specific amount of fertilizer. After six weeks, the plants that received fertilizer are, on average, taller than those that did not. Does this study provide evidence that fertilizer causes increased plant growth? Explain your answer.

12. In a laboratory experiment, participants are randomly assigned to consume either a caffeinated beverage or a caffeine-free beverage that looks and tastes the same. After 30 minutes, reaction times are measured. Participants who consumed caffeine had faster reaction times on average. Does this study provide evidence that caffeine causes faster reaction times? Explain your answer.

13. There is a correlation between police expenditure and crime rate. Does this mean that spending more money on police causes the crime rate to decrease? Explain your answer. If you believe another variable may be influencing both, identify a possible confounding variable.

14.	There is a correlation between tobacco sales and alcohol sales. Does this mean that using tobacco causes a person to also drink alcohol? Explain your answer. If a third variable could help explain the relationship, describe a possible confounding variable.

15.	There is a correlation between the average temperature in a location and the mortality rate from breast cancer. Does this mean that higher temperatures cause more women to die of breast cancer? Explain your answer. Identify a possible confounding variable, if one exists.

16.	There is a correlation between the length of time a tableware company polishes a dish and the price of the dish. Does this mean that the time a plate is polished determines the price of the dish? Explain your answer. If appropriate, suggest a possible confounding variable that could explain the relationship.

## Inference for Regression and Correlation {#sec-inference-regression}

### Prediction Interval

The idea behind regression is to find an equation that relates the response variable to the predictor variables, and then use that equation to predict values of the response variable from values of the predictor variables. But how do you know how good that estimate is?
...
Because of this variability, it is usually more informative to give a range of plausible values for the response variable rather than a single predicted value. A **prediction interval** provides such a range. It gives an interval of values that is likely to contain the response value for a new observation with specified predictor variable values, along with a stated level of confidence.

::: {#exm-find-pred-interval}
### Obtaining a Prediction Interval

The [beer]{.data} dataset contains information on the alcohol content and number of calories for a sample of 12-ounce beers. The first six rows of this dataset are shown in @tbl-Beer.

Is there a linear relationship between alcohol content and the number of calories in a 12-ounce beer? Using this dataset, find a 95% prediction interval for the number of calories when the alcohol content is 7%, and interpret the interval in context.
:::
...
It is important to check the assumptions under which a prediction interval is valid. These assumptions are given in @sec-assumption-validity-inference (Assumptions for Validity of Inference).
...
## Testing a Claim of Existence of a Correlation

The [beer]{.data} dataset contains information on the alcohol content and number of calories for a sample of 12-ounce beers. The first six rows of this dataset are shown in @tbl-Beer.  

Is there a linear relationship between alcohol content and the number of calories in a 12-ounce beer? Using this dataset, test at the 5% significance level whether a correlation exists.
...
The variables that we are considering are $x$ = alcohol content in the beer and $y$ = calories in 12-ounce beer, and the null and alternative hypotheses are
...
The default significance level is 0.05, so you do not need to change. If a significance level other than 5% is required, enter the desired value as a decimal in the box labeled [Significance Level]{.des}.
...
To check the constant variance assumption, we examine the residual-versus-fit plot: if the residuals are scattered with roughly the same vertical spread around the horizontal line $y = 0$ across all fitted-values, the assumption is reasonable. A funnel-shaped pattern suggests unequal variances. 
...
1.  When an anthropologist finds skeletal remains, they need to estimate the height of the person. The height of a person (in cm) and the length of their metacarpal bone 1 (in mm) were collected. The data are in the [height_vs_metacarp]{.data} dataset, and the first six rows of the data are shown in @tbl-Metacarpal ("Prediction of height," 2013).

    a.  Compute and interpret a 99% prediction interval for the height of a person with a metacarpal length of 44 mm.

    b.  Conduct a hypothesis test to determine whether there is a linear relationship between the length of metacarpal bone 1 and the height of a person. Write the null and alternative hypotheses in the context of this problem, test at the 1% significance level, and state your conclusion in context.

    c.  Examine the residual-versus-fit plot from your fitted linear regression model. Does the plot suggest that the assumption of constant variance is satisfied? Explain.

    d.  Examine the normal probability plot for the residuals. Does the plot suggest that the assumption of normality of the errors is satisfied? Explain.
...
2.  @tbl-House contains the first six values of house price and the amount of rental income per year that the house generates. The full dataset is available in [house_vs_rent]{.data}.

    a.  Compute and interpret a 95% prediction interval for the annual rental income on a house worth $230,000.

    b.  Conduct a hypothesis test to determine whether there is a linear relationship between house value and annual rental income. Write the null and alternative hypotheses in the context of this problem, test at the 5% significance level, and state your conclusion in context.
...
3.  The World Bank collects information on life expectancy for each country and the fertility rate per woman in each country ("Fertility rate," 2013). The first six rows of the [Life_exp_fert_rate]{.data} dataset for the year 2011 are shown in @tbl-Fertility.

    a.  Compute and interpret a 99% prediction interval for the life expectancy of a country with a fertility rate of 2.7.

    b.  Conduct a hypothesis test to determine whether there is a linear relationship between fertility rate and life expectancy. Write the null and alternative hypotheses in the context of this problem, test at the 1% significance level, and state your conclusion in context.
...
4.  The World Bank collected data on the percentage of gross domestic product (GDP) that a country spends on health expenditures (Current health expenditure (% of GDP), 2019), the fertility rate of the country (Fertility rate, total (births per woman), 2019), and the percentage of women receiving prenatal care (Pregnant women receiving prenatal care (%), 2019). The data for the countries where this information is available are in the [fertility_prenatal]{.data} dataset, and the first six rows are shown in @tbl-Fert_prenatal.
    ...
    a.  Compute and interpret a 95% prediction interval for the percentage of women receiving prenatal care for a country that spends 5.0% of GDP on health expenditures.

    b.  Conduct a hypothesis test to determine whether there is a linear relationship between the percentage of GDP spent on health expenditures and the percentage of women receiving prenatal care. Write the null and alternative hypotheses in the context of this problem, test at the 5% significance level, and state your conclusion in context.
...
5.  The height and weight of baseball players are in the [baseball]{.data} dataset, and the first six rows of the data are shown in @tbl-Baseball.

    a.  Compute and interpret a 95% prediction interval for the weight of a baseball player who is 75 inches tall.

    b.  Conduct a hypothesis test to determine whether there is a linear relationship between height and weight of baseball players. Write the null and alternative hypotheses in the context of this problem, test at the 5% significance level, and state your conclusion in context.
...
6.  Different species have different body weights and brain weights. The [body]{.data} dataset contains data on these two variables, and the first six rows are shown in @tbl-Body.

    a.  Compute and interpret a 99% prediction interval for the brain weight of a species that has a body weight of 62 kg.

    b.  Conduct a hypothesis test to determine whether there is a linear relationship between body weight and brain weight. Write the null and alternative hypotheses in the context of this problem, test at the 1% significance level, and state your conclusion in context.
...
7.  A sample of hot dogs was taken, and the amount of sodium (in mg) and calories were measured ("Data hotdogs," 2013). The data are in the [hotdog]{.data} dataset, and the first six rows are shown in @tbl-Hotdog.

    a.  Compute and interpret a 95% prediction interval for the amount of sodium in a beef hot dog that contains 170 calories.

    b.  Conduct a hypothesis test to determine whether there is a linear relationship between the amount of calories and the amount of sodium in hot dogs. Write the null and alternative hypotheses in the context of this problem, test at the 5% significance level, and state your conclusion in context.
...
8.  Per capita income (in 1960 dollars) for European countries and the percent of the labor force working in agriculture in 1960 are in the [agriculture]{.data} dataset. The first six rows are shown in @tbl-Agriculture ("OECD economic development," 2013).

    a.  Compute and interpret a 90% prediction interval for the per capita income of a country that has 21% of its labor force working in agriculture.

    b.  Conduct a hypothesis test to determine whether there is a linear relationship between the percent of the labor force working in agriculture and per capita income. Write the null and alternative hypotheses in the context of this problem, test at the 5% significance level, and state your conclusion in context.
...
**Code book for Dataset Agriculture is below @tbl-Agriculture.**

</div>

9.  Cigarette smoking and cancer have been linked. The number of deaths per one hundred thousand from bladder cancer and the number of cigarettes sold per capita in 1960 are given in the [cancer_1]{.data} dataset, with the first six rows shown in @tbl-Cancerbladder ("Smoking and cancer," 2013).

    a.  Compute and interpret a 99% prediction interval for the number of deaths from bladder cancer when cigarette sales were 20 per capita.

    b.  Conduct a hypothesis test to determine whether there is a linear relationship between the number of cigarettes sold per capita and the number of deaths from bladder cancer. Write the null and alternative hypotheses in the context of this problem, test at the 1% significance level, and state your conclusion in context.
...
10. The weight of a car can influence the mileage that the car can obtain. A random sample of car weights and mileage values is given in the [cars]{.data} dataset. The first six rows of the data are shown in @tbl-Cars ("us auto mileage," 2019).

    a. Compute and interpret a 95% prediction interval for the mileage of a car that weighs 3800 pounds.

    b. Conduct a hypothesis test to determine whether there is a linear relationship between car weight and mileage. Write the null and alternative hypotheses in the context of this problem, test at the 5% significance level, and state your conclusion in context.
