...
In this course, we work primarily with numerical data, so our focus will be on the mean and median as measures of center. The mode is more commonly used with categorical data, where it identifies the most frequently occurring category. While the mean and median both require numerical values, the key difference is that the mean uses every data value in its calculation, making it sensitive to extreme values, whereas the median depends only on the position of the middle value.
...
![Rguroo dataset logo](Rguroo_Icons/Rguroo_repo_logo.png) The dataset for this example is available in the Rguroo dataset repository [Kozak]{.repo}, with the dataset name [cats]{.data}. @tbl-Cats displays the first six rows of the dataset. A complete description of the variables is provided in the [dataset code book](#code-book-cats) that follows.
:::
...
Looking at the dataset for cats weights, @tbl-Cats, you see that there are several variables. The code book for the [cats]{.data} is below.
...
[sex]{.var}: Factor with levels "F" and "M".

[Bwt]{.var}: body weight in kg.

[Hwt]{.var:} heart weight in g.
...
To find the effect of extreme values on the mean and median, we first need to identify the maximum and minimum values. The steps to find the minimum and the maximum in Rguroo are similar to finding the mean and median described [here](#Calculating-summary-stats-in-Rguroo). In the **Statistics** section of the dialog, select the checkbox for [Minimum]{.des} and [Maximum]{.des}.
...
![Rguroo dataset logo](Rguroo_Icons/Rguroo_repo_logo.png) The dataset for this example is available in the Rguroo dataset repository [Kozak]{.repo}, with the dataset name [Tornado_OK]{.data}. @tbl-Tornado displays the first six rows of the dataset. A complete description of the variables is provided in the [dataset code book](#code-book-tornado) that follows.
:::
...
7.  To determine if Reiki is an effective method for treating pain, a pilot study was carried out in which a certified second-degree Reiki therapist provided treatment to volunteers. Pain was measured using two scales — a visual analogue scale (VAS) and a Likert scale — immediately before and after the Reiki treatment (Olson & Hanson, 1997). The data are in @tbl-Reiki.
...
Since pain was measured on the same individuals both before and after treatment, this is **paired data**. To analyze paired data, we calculate the difference between the before and after measurements for each person. This difference represents the effect of the treatment.
...
**(a)**: Create a new dataset called [Newreiki]{.data} that adds a column for the difference between the before and after VAS scores. Name this new variable [vas.diff]{.var}. Then find the mean and standard deviation of [vas.diff]{.var}.

**(b)**: Using the same approach, create a variable called [likert.diff]{.var} for the difference between the before and after Likert scores. Find the mean and standard deviation of [likert.diff]{.var}.

**(c)**: Compare the results from Part (a) and Part (b). What do the mean differences tell you about the effectiveness of the Reiki treatment? Do the VAS and Likert scales give similar conclusions?
...
For instance, a score at the $90^{th}$ percentile means that 90% of the scores were at or below this score — in other words, the person did the same as or better than 90% of the test takers. A score at the $70^{th}$ percentile means that 70% of the scores were at or below this score.

It is important to understand that a percentile is a ranking, not a score. If a test was out of 100 points and you scored at the $80^{th}$ percentile, that does not mean you earned 80 points. It means you scored the same as or better than 80% of the people who took the test. If all the scores were really low, you could have still failed the test. On the other hand, if many of the scores were high you could have gotten a 95% or more. The percentile tells you how you compared to others, not what your actual score was.
...
::: {.callout-note appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
To create a side-by-side boxplot separated by the sex factor, move the [sex]{.var} factor variable to the Selected column in the Factor Variables section. ![Boxplot dialog in Rguroo](Rguroo_dialogs/Numerical_description/boxplot_cats_sex_dialog.png){width="500px"}
:::
...
![Rguroo dataset logo](Rguroo_Icons/Rguroo_repo_logo.png) The dataset for this example is available in the Rguroo dataset repository [Kozak]{.repo}, with the dataset name [nerve]{.data}. @tbl-Nerve displays the first six rows of the dataset. A complete description of the variables is provided in the [dataset code book](#code-book-nerve) that follows.
...
Using the [nerve]{.data} dataset:

a.  Create a density plot of the [time]{.var} variable. Describe the shape of the distribution.
b.  Find the mean, median, standard deviation, and five-number summary.
c.  Based on the shape of the distribution, which measure of center is more appropriate — the mean or the median? Explain your reasoning.
d.  Create a boxplot of the [time]{.var} variable. Are there any outliers? How does the boxplot support your answer to part (c)?
:::

::::{.callout-tip .solution-callout collapse="true" icon=false}
## 🔎 Solution

**Part (a):** First, create a density plot to visualize the distribution.
...
4.  Move the [time]{.var} variable to the Selected column.\
...
::: {.callout-note appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
![Density plot dialog in Rguroo](Rguroo_dialogs/Numerical_description/nerve_density_dialog.png){width="500px"}
:::
::::
...
The distribution is skewed right. Most of the times between successive nerve pulses are around 5 to 10 (in 1/50 seconds), but there are some values as high as 60.

**Part (b):** Using the summary statistics from Rguroo (@fig-Pulse-summary):
...
The mean is 11 (1/50 seconds), the median is 7.5 (1/50 seconds), and the standard deviation is 10.5 (1/50 seconds). The five-number summary is: minimum = 0.5, Q1 = 3.5, median = 7.5, Q3 = 15, and maximum = 69 (1/50 seconds).

**Part (c):** Because the distribution is skewed right, the median (7.5) is a more appropriate measure of center than the mean (11). The mean is pulled to the right by the long tail of high values, making it larger than the median. Since the median is resistant to extreme values, it better represents the typical time between nerve pulses.

**Part (d):** Create a boxplot using the instructions given [here](#Creating-a-boxplot-in-Rguroo).
...
The boxplot reveals multiple high outliers above the upper fence. The IQR is Q3 − Q1 = 15 − 3.5 = 11.5 (1/50 seconds), indicating substantial variability. The outliers and the right skew visible in both the density plot and the boxplot confirm that the median is the better measure of center for this dataset — the extreme values pull the mean higher but do not affect the median.
::::
...
6.  The lengths (in kilometers) of rivers on the South Island of New Zealand and what body of water they flow into are listed in table @tbl-Length (Lee, 1994). Find the five-number summary and interquartile range (IQR) for the lengths of rivers that go to the Pacific Ocean and ones that go to the Tasman Sea, and draw a boxplot of both.
...
8.  The dataset Pulse @tbl-Pulse contains various variables about a person including their pulse rates before the subject exercised and after after the subject ran in place for one minute. Create a dataset that contains only people who drink alcohol, but do not smoke. Then find the five number summary and draw a boxplot for both males and females separately.
