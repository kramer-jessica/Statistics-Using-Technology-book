...
### Example: Finding the Mean and Median using Rguroo {#example-mean-median-cats}

Suppose a vet wants to find the average weight of cats. The weights (in kg) of 144 cats are  given in a dataset named [cats]{.data}. The first six rows of this dataset are shown in @tbl-Cats.
...
```{r cats1-data, echo = FALSE, eval = FALSE}
#??cats
```

Click to expand the box below to see instructions to import and view the [cats]{.data} dataset in your **Data** Toolbox in Rguroo.

::: {#Importing-cats-to-Rguroo .callout-note appearance="simple" collapse="true" icon="none" title="![](Rguroo_Icons/R_circle_logo.svg){width=22px style='vertical-align:middle;'}  Importing cats to Data Toolbox"}
1.  Go to the **Data** toolbox.
2.  From the [Data Import]{.dpd} dropdown, select [Dataset Repository]{.fun}.
3.  In the top search box, type [MASS]{.typein}, then select the [MASS]{.repo} repository.
4.  In the middle search box, type [cats]{.typein}, and select the [cats]{.data} dataset that appears in the lower panel.\
5.  Click the [Import]{.button} button. The dataset will be imported into your Rguroo account.
6.  Click the [Close]{.button} button to close the Rguroo dialog.
7.  To view the dataset, double-click [cats]{.data} under the **Data** toolbox list.
:::

Looking at the data frame for cats weights, @tbl-Cats, you see that there are several variables. A Code Book describes the dataset, explains what the variables are including the units, and the source of the data frame. The code book for the [cats]{.data} is below.
...
5.  In the **Statistics** section of the dialog, select the checkbox for Mean and Median.\
6.  Click the preview icon ![preview icon](Rguroo_Icons/preview_inline.png) to see the summary statistics.
...
```{r statistics-cats2, echo = FALSE, evaluate = FALSE}
...
### Example: Finding Mean and Median with Factor
...
6.  In the **Statistics** section of the dialog, select the checkbox for Mean and Median.\
...
There are many different summary statistics that can be found. An example is the minimum and maximum value. In this example, you will see how to find the min and max values and then filter them out of a dataset to see what effect they have on the mean and median.

### Example: Effect of Extreme Values on Mean and Median {#example-effect-extreme-values}
...
::: {.callout-note appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
![Logical Expression Dialog in Rguroo](Rguroo_dialogs/Numerical_description/subset_nomax_cats.png){width="500px"} ![Data Subset Dialog in Rguroo](Rguroo_dialogs/Numerical_description/logical_subset_nomax.png){width="500px"} ![Save As... Dataset View](Rguroo_outputs/Numerical_Description/save_data_nomax.png)
:::
::::

```{r filter-cats-data,warning=FALSE, echo = FALSE, eval = FALSE}
...
```{r filter-cats1-data, echo = FALSE, eval = FALSE}
nomin<-filter(cats, Bwt>2)
```

<!-- The \<- is the way to indicate to r what the dataset nomin is equivalent to what follows the symbol. Notice that it doesn't look like anything happened, but new datasets were created in the background.-->
...
```{r statistics-nomax, eval = FALSE, echo = FALSE}
df_stats(~Bwt, data=nomax, mean, median)
```

The mean without the maximum value is 2.70 kg, and the median is 2.7 kg.

```{r statistics-nomin, eval = FALSE, echo = FALSE}
df_stats(~Bwt, data=nomin, mean, median)
```

The mean without the minimum value is 2.75 kg, and the median is 2.7 kg.

From [this](#example-effect-extreme-values) example, the mean of the [cats]{.data} dataset with all the values is 2.72 kg, while the median is 2.7 kg. Notice that when the maximum value was excluded from the dataset, the mean decreased slightly but the median didn't change, and when the minimum value was excluded from the dataset, the mean increased a little but the median didn't change. This shows that the mean is affected by extreme values, while the median is not. We say the median is a resistant measure of center because it isn't affected by extreme values as much.
...
<!--
@fig-centers-dist**:** **Mean, Median, Mode as Related to a Distribution**
-->
...
4.  Enter your data (score and weights) in the two columns.\
5.  Enter a name for your dataset in the [Save As]{.des} textbox on the top, and click the [Save as]{.button} button to save your dataset. For this example, name the dataset [biology_grade]{.data}.
...
```{=html}
<!-- The commands for finding the weighted mean using RStudio is as follows:
...
```{r weighted-mean, echo = FALSE, eval = FALSE}
x<-c(95, 83, 76, 84) 
w<-c(.15, .25, .25, .35) 
weighted.mean(x,w) 
```
...
```{r weighted-mean1, echo = FALSE, eval = FALSE}
x<-c(8, 9, 2, 1, 8) 
w<-c(20, 18, 6, 4, 2) 
weighted.mean(x,w)
```
...
```{r weighted-mean2, echo = FALSE, eval = FALSE}
x<-c(6, 8, 9, 10, 10) 
w<-c(20, 18, 6, 4, 2) 
weighted.mean(x,w)
```

The weighted mean can be calculated in the same fasion as part (a). In the [Basic Summary Statistics]{.dialog} dialog box select the [faculty_ratings]{.data} from the [dataset]{.dpd} dropdown, select [faculty_b]{.var} in the [Numerical]{.dpd} dropdown, and select [weight]{.var} in the [frequency]{.dpd} dropdown as shown in figure
...
Source Ryan, B. F., Joiner, B. L., & Ryan, Jr, T. A. (1985). Cholesterol levels after heart attack. Retrieved from [here](https://gksmyth.github.io/ozdasl/general/cholest.html)

References Ryan, Joiner & Ryan, Jr, 1985

2.  The lengths (in kilometers) of rivers on the South Island of New Zealand and what body of water they flow into are given in a dataset named [length]{.data}. The first six rows of this datset are listed in @tbl-Length (Lee, 1994). Find the mean and median length of rivers that flow into the Pacific Ocean and the mean and median length of rivers that flow into the Tasman Sea.
...
Source Lee, A. (1994). Data analysis: An introduction based on r. Auckland. Retrieved from [here](https://gksmyth.github.io/ozdasl/oz/nzrivers.html)
...
d.  If you were the CEO, who is answering concerns from the union that employees are underpaid, which average (mean or median) using the complete dataset of the complete dataset would you prefer? Why?
...
Variability is an important idea in statistics. If you were to measure the height of everyone in your classroom, the resulting observations will generally have different values. That means not every student has the same height. Thus there is variability in people's heights. If you were to take a sample of the income level of people in a town, every sample gives you different information. <!-- There is variability between samples too. --> Variability describes how the data are spread out. If the data are very close to each other, then there is low variability. If the data are very spread out, then there is high variability. How do you measure variability? It would be good to have a number that measures it. This section will describe some of the different measures of variability, also known as measures of variation.

In the [Example: Finding the Mean and Median using Rguroo](#example-mean-median-cats), the average weight of a cat was calculated to be 2.72 kg. How much does this tell you about the weight of all cats? Can you tell if most of the weights were close to 2.72 kg or were the weights really spread out? The highest weight and the lowest weight are known, but is there more that you can tell? All you know is that the center of the weights is 2.72 kg.
...
Unfortunately, the range does not provide a very accurate picture of variability. A better way to describe how the data are spread out is needed. Instead of looking only at the distance between the highest and lowest values, we can look at how far each data value is from the mean. This distance is called a **deviation**.

You might want to find an average of these deviations. This leads to calculating a value called the **variance**. The symbol for the population variance is $\sigma^2$, where $\sigma$ is the Greek letter sigma. The population variance is the average squared distance of each data value from the mean of the data. 

In practice, however, people usually use and report the square root of the variance, which is called **standard deviation**. <!-- NOT TRUE This gives you the average distance from the mean. --> The population standard deviation is denoted with the symbol $\sigma$. The reason for taking the square root is that the variance is measured in squared units. For example, if the data are measured in kilograms (kg), then the variance is measured in kg-squared. The standard deviation, on the other hand, is measured in the same units as the original data. So if the data are in kg, the standard deviation is also in kg, which makes it easier to interpret.

<!-- NOT TRUE The standard deviation is the average (mean) distance from a data point to the mean. It can be thought of as how much a typical data point differs from the mean.-->

To estimate the population variance, we use the so called **sample variance**. The sample variance is denoted by $s^2$.
...
The **population variance** formula: $\sigma^2 = \frac{\sum\left(x-\mu \right)^2}{N}$, where  $\sigma^2$ represents the population variance, $\mu$ is the population mean, and $N$ is the size of the population.

The **population standard deviation** formula: $\sigma =\sqrt{ \frac{\sum\left(x-\mu \right)^2}{N}}$

Both the sample variance and sample standard deviation can be found using technology.
...
For the dataset [cats]{.data} (@tbl-Cats) find the variance and standard derivation for weight of cats. Then find the variance and standard deviation separated by sex of the cat.

#### **Solution**

 When using Rguroo, you would perform the similar steps as described in calculation mean and median[here](#Calculating-summary-stats-in-Rguroo), and select [standard deviation]{.def} and [variance]{.def}, as shown in the figure @fig-sd-dialog.
...
```{r statistics-cats, echo = FALSE, eval = FALSE}
...
```{r statistics-cats-sex, eval = FALSE, echo = FALSE}
...
In general a "small" variance and standard deviation means the data is close together (more consistent) and a "large" variance and standard deviation means the data is spread out (less consistent). Sometimes you want consistent data and sometimes you don't. As an example if you are making bolts, you want the lengths to be very consistent so you want a small standard deviation. On the other hand, if you are administering a test to evaluate pilot ability, you want the test scores to show enough variability to distinguish strong candidates from weaker ones. If nearly everyone scores about the same, the standard deviation will be small and it will be difficult to tell who performs better. A larger standard deviation indicates that scores are more spread out, making it easier to differentiate between high-performing and low-performing candidates.

What do “small” and “large” standard deviation mean? To a bicyclist whose average speed is 20 mph, $s = 20$ mph is huge. To an airplane whose average speed is 500 mph, $s = 20$ mph is small. The “size” of the variation depends on the size of the numbers in the problem and the mean. Another situation where it makes sense to describe a standard deviation as small or large is when comparing two different samples, such as in [Example: Finding the Standard Deviation]. When two samples are measured on the same scale, the sample with the smaller standard deviation shows less spread in its data than the sample with the larger standard deviation.

Many textbooks present an alternative computational formula for calculating the standard deviation. While this formula can be efficient for performing calculations, it does not provide much insight into what the standard deviation represents or how it is constructed. Historically, this computational formula was useful when standard deviations were calculated by hand, since it simplified the arithmetic. Today, however, computers perform these calculations quickly and accurately. For this reason, the computational formula is no longer necessary for routine work. To develop a clear understanding of what the standard deviation measures, it is better to focus on the definition-based formula. When a numerical answer is needed, using a computer is the most practical approach.


### **Use of Standard Deviation**
<!--
One of the uses of the standard deviation is to describe how values are a population is distributed. This describes where much of the data is for most distributions. A general rule is that about 95% of the data is within 2 standard deviations of the mean. This is not perfect, but is works for many distributions. There are rules like the empirical rule and Chebyshev's theorem that give you more detailed percentages, but 95% in 2 standard deviations is a very good approximation.
...
For data that are not symmetric, other rules can still be used. Chebyshev’s rule gives a minimum proportion of the data that must lie within a certain number of standard deviations of the mean, regardless of the overall shape of the distribution. The results from this rule are usually less precise than those from the empirical rule, but they apply to a much wider range of situations.

### Example: the general rule

In the dataset [Tornado_OK]{.data}, the U.S. Weather Service has provided information about the total monthly/annual number of reported tornadoes in Oklahoma for the years 1950 to 2018 (US Department of Commerce & Noaa, 2016). The first six rows of the dataset are shown in @tbl-Tornado.
...
Import the [Torando_OK]{.data} dataset from the ??? repository into your Data toolbox on Rguroo.

Variable: $x$ = number of **annual** tornadoes in Oklahoma

Find the mean and standard deviation as in @fig-tornado-ex:

```{r statistics-tornado, echo = FALSE, eval = FALSE}
df_stats(~Annual, data=Tornado, mean, sd)
```

```{r,echo=FALSE}
#| fig-alt: "Rguroo Dialog Box and Output for Mean and Standard Deivation of Annual Tornados"
#| warning: FALSE
#| label: fig-tornado-ex
#| fig-cap: "Mean and Standard Deviation of Annual Tornados"
#| fig-subcap: 
#|   - "Basic Summaries Dialog"
#|   - "Basic Summaries Output"
#| out-width: "70%"
...
To answer this question, first find how many standard deviations 45 is from the mean. From [Example: the general rule], we know $\mu=56$ and $\sigma=27.6$. For $x$=45, $z=\frac{45-56}{27.6}=-0.399$
...
Source Pulse rates before and after exercise. (2013, September 25). Retrieved from [here](https://gksmyth.github.io/ozdasl/oz/ms212.html)
...
Source Olson, K., & Hanson, J. (1997). Using reiki to manage pain: a preliminary report. Cancer Prev Control, 1(2), 108-13. Retrieved from [here](http://www.ncbi.nlm.nih.gov/pubmed/9765732)
...
4.  Click on the plus sign icon ![plus icon](Rguroo_Icons/add.png) in the Variable column and type [vas.diff]{.var}. The variable’s name appears in the list labeled Retuned Variable on the right side.\
...
6.  When you are done with your selection, click the preview icon ![preview icon](Rguroo_Icons/preview_inline.png) to see a preview of the scatterplot.
...
Source Annual maximums of daily rainfall in Sydney. (2013, September 25). Retrieved from [here](https://gksmyth.github.io/ozdasl/oz/sydrain.html).
...
To find the quartiles in Rguroo, you use the same steps as finding the mean, standard deviation, and variation in Summary Stasitic as described [here](#Calculating-summary-stats-in-Rguroo).
...
A value that is less than $Q1-1.5*IQR$ (this value is often referred to as a **lower fence**) is considered an outlier.

Similarly, a value that is more than $Q3+1.5*IQR$ (the **upper fence**) is considered an outlier.

A boxplot (or box-and-whisker plot) is a graphical display of the five-number summary. It can be drawn vertically or horizontally. In the horizontal form, the plot consists of a rectangular box that extends from $Q_1$ to $Q_3$, with a line inside the box marking the median. Lines called whiskers extend horizontally from each end of the box to the minimum and maximum values. The horizontal axis is a number line that shows the values of the variable being displayed, with appropriate tick marks and a label. A title is often included above the plot.

An alternate form of a boxplot, known as a modified boxplot, shortens the whiskers so that the left whisker extends only to the smallest value greater than the **lower fence** and the right whisker extends only to the largest value less than the **upper fence**. Any values beyond these fences are considered outliers and are shown separately using markers such as dots, circles, or asterisks.

If the data are **symmetrical**, then the box plot will be visibly symmetrical around the median line. If the distribution is skewed to the left or to the right, the whisker on the skewed side will tend to be longer, and the median will be shifted away from the center of the box in the opposite direction. <!-- THIS IS NOT TRUE SINCE THIS CAN HAPPEN FOR NON_UNIFORM DISTRIBUTIONS If the plot is symmetrical, and the four quartiles are all about the same length, then the data are likely a near **uniform** distribution. --> If a boxplot is symmetrical and the whiskers are noticeably longer than the distances from $Q_1$ to the median and from the median to $Q_3$, the distribution may be bell-shaped, with more data concentrated near the center and thinner tails. However, this pattern alone is not sufficient to conclude that the distribution is bell-shaped.
...
```{r statistics-cats1, echo = FALSE, eval = FALSE}
...
The five-number summary separated based on gender can be found in a similar way to [Example: Five-number Summary and Boxplot]:

```{r statistics-cats2-sex, echo = FALSE, eval = FALSE}
...
Source Time between nerve pulses. (2019, July 3). Retrieved from here (https://gksmyth.github.io/ozdasl/general/nerve.html)
...
**Before you begin:** Make sure you have already imported the [nerve]{.data} dataset into your **Data** Toolbox, located in the [Statistics Using Technology - Kozak]{.repo}.
...
4.  Move the [pulse]{.var} variables to the Selected column.\
...
::: {.rg-iframe-wrapper tabindex="0"}
<iframe src="Rguroo_html_output/Numerical_description/Histogram_7/Histogram_7.html" class="rg-iframe" title="Denisty Plot for Successive Nerve Pulses">
