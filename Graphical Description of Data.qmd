...
Remember, qualitative data are words describing a characteristic of the individual. Once you’ve gathered raw data, the real work begins. A long list of observations doesn’t tell you much on its own. Raw data is usually messy, unstructured, and difficult to understand or use. The next step is to organize the data in a way that reveals patterns and meaning. A simple way to organize qualitative data is to create a frequency distribution or relative frequency distribution. Both of these are tables which organize non‑numerical categories into groups and shows how often each category occurs.

A **frequency distribution for qualitative data** is a table that lists each category with its corresponding frequency.

A **relative frequency distribution for qualitative data** is a table that lists each category with its corresponding relative frequency. Relative frequencies can be in the form of a proportion or a percentage.
...
Data was collected for two semesters in a statistics class. @tbl-Class shows the variables and the first 6 rows of the data set. The data sets are usually larger than what is shown. Create a frequency distribution and relative frequency distribution for vehicle type.
...
](Rguroo_dialogs/Graphical_Description_of_Data/dialog_frequency_distribution_vehicle.png){width=500px}
...
**Optional** You can change the title and names for the x and y axes in the Barplot dialog box.  To do this, simply type in the titles for each in the **Label** boxes.
...
Description of @fig-class_data_point is a pie chart of type of vehicles owned by students (Toyota, Subaru, none, Nissan, Mercury, Jeep, Honda, Ford, Dodge, Chevrolet, and Buick, Hyundai, and Audi). Each vehicle percentage is displayed in the legend on the right.
...
Bell-shape means that you can fold the graph in half down the middle and the two sides will line up. You can think of the two sides as being mirror images of each other. Skewed means one "tail" of the graph is longer than the other. The graph is skewed in the direction of the longer tail (backwards from what you would expect). A uniform graph has all the bars the same height.
...
![Scatter plot dialog showing settings for creating scatter plot of average sleep versus GPA.
](Rguroo_dialogs/Graphical_Description_of_Data/dialog_sleepstudy.png){width=500px}
