...
There are two types of variables: categorical (also called qualitative or factor) and numerical (also called quantitative). **Categorical variables** or **factors** describe qualities, categories, or labels rather than numerical amounts. Their values place observations into groups based on characteristics or attributes; for example, eye color, political party, or type of car. **Numerical variables** are numerical measurements that represent amounts or counts. Their values can be ordered, compared, and used in arithmetic operations, such as computing averages, for example, height, temperature, or number of siblings.
...
![Random selection dialog showing settings for taking a simple random sample.](Rguroo_dialogs/Statistical_Basics/dialog_srs_gender.png){width="500px"}
:::
::::
...
::: {.callout-tip .inner-callout appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
![Random selection dialog showing settings for taking a simple random sample.](Rguroo_dialogs/Statistical_Basics/dialog_stratified_gender_vehicle.png){width="500px"}
:::
::::
...
@fig-sampling-compared compares simple random, stratified, and cluster samples using college dormitories. Panel (a) illustrates a simple random sample by selecting individual rooms scattered across both dorms. Panel (b) illustrates a stratified sample by separately sampling rooms from each dorm type. Panel (c) illustrates a cluster sample depicted by selecting entire floors within each dorm, and then all rooms are selected.
...
::: {.callout-tip .inner-callout appearance="simple" collapse="true" icon="none" title="Click here to see the Rguroo dialog"}
![Random selection dialog showing settings for taking a simple random sample.](Rguroo_dialogs/Statistical_Basics/dialog_systematic.png){width="500px"}
:::
::::
...
Observational studies are research studies in which the investigator observes and collects data without manipulating variables. They are commonly classified into three types. A **cross-sectional study** collects data from a population at a single point in time (or over a short period of time), providing a snapshot of characteristics or behaviors as they currently exist. A **case-control study** begins with individuals who have a particular outcome (cases) and compares them with similar individuals who do not have the outcome (controls). Researchers then look backward using past records, interviews, or historical data to investigate possible exposures or risk factors. Case-control studies are typically **retrospective**. A **cohort study** follows individuals or groups into the future, collecting data at multiple points to observe how certain factors influence outcomes as they develop. Cohort studies are often **longitudinal or prospective.** Many observational studies collect data through surveys, which are sets of questions designed to gather information. Survey questions must be carefully written to avoid introducing bias.
...
A city notices that neighborhoods with more parks tend to have lower crime rates. Based on this observation, city officials conclude that building more parks will reduce crime. What is the potential problem with this conclusion?
:::
